...
class Purchase_consume_report extends CI_Controller {
	...
	public function index()
	{
		...
		#----load view----------#
		$data["title"] = "Itemwise Analysis Report";	
		$this->load->view($this->session->userdata('language')."/Purchase_consume_report/search",$data);
	}
	...
	public function report()
	{
        ...
        $from_date =$data['from_date']=$this->input->post("from_date");
		 ...
		 // if($plant !='All'){ $where_plant = " and tbl_issue_goods.tank_id='$plant'  "; }else{ $where_plant =""; }
		   	...
		   	// $data['report'] = $this->db->query("SELECT tbl_issue_goods.*,tblacode.aname as customer FROM `tbl_issue_goods` inner join tblacode on tblacode.acode=tbl_issue_goods.issuedto where  tbl_issue_goods.issuedate Between '$from_date' and '$to_date' and tbl_issue_goods.sale_point_id='$sale_point_id'     order by tbl_issue_goods.issuenos asc")->result_array();
	 ...
	 //pm($data['cyl_list']);
			...
			if (!$data['report']) {
			 	$data["title"] = "Itemwise Analysis Report";
	            $this->load->view($this->session->userdata('language')."/Purchase_consume_report/detail",$data);
	        } else {
	            $this->session->set_flashdata('err_message', 'No Record Found.');
	            redirect(SURL . 'Purchase_consume_report/');
...
if($this->input->server('REQUEST_METHOD') == 'POST'){
	  ...
	  //pm($data);
		...
		$this->load->view($this->session->userdata('language')."/Purchase_consume_report/pdffile",$data);
