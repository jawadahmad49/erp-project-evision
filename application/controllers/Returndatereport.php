...
class Returndatereport extends CI_Controller {
	...
	public function __construct() {
        ...
        $this->load->model(array(
            "mod_returnreport","mod_common","mod_customer","mod_salereturn"
        ));
	...
	public function index()
	{
		...
		$data["filter"] = '';
		#----load view----------#
		$data["title"] = "Return B/W Date Report";	
		$this->load->view($this->session->userdata('language')."/Returndatereport/search",$data);
	}
	...
	public function details()
	{
		if($this->input->server('REQUEST_METHOD') == 'POST'){
			...
			$data['report']=  $this->mod_returnreport->get_details($this->input->post());
			//pm($data['report']); exit;
			...
			if ($data['report']) {
	            ...
	            //redirect(SURL . 'Returndatereport/detail',$data);
	            $data["title"] = "Return B/W Date Report";
	            $this->load->view($this->session->userdata('language')."/Returndatereport/detail",$data);
	        } else {
	            $this->session->set_flashdata('err_message', 'No Record Found.');
	            redirect(SURL . 'Returndatereport/');
	        }
	    }else{
	        //$data["filter"] = 'add';
	        $data["title"] = "Return B/W Date Report";    			
			$this->load->view('Returndatereport/detail',$data);
		}
	}
	...
	public function detail($id){
		...
		#----load view----------#
		$data["title"] = "Return B/W Date Report";
		$this->load->view($this->session->userdata('language')."/Returndatereport/single",$data);
		}
	}
	...
	public function edit($id){
		if($id){
			...
			$this->load->view($this->session->userdata('language')."/Returndatereport/add", $data);
		}
		/* Update Data */
		if($this->input->server('REQUEST_METHOD') == 'POST'){
