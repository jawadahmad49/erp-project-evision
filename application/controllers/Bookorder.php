...
class BookOrder extends CI_Controller
{
	...
	function getRemarks()
	{
		...
		$direct_customer = $_POST['direct_customer'];

		if ($direct_customer > 0) {
			$remarks = $this->db->query("SELECT remarks FROM `tbl_lead_generation` where customer_acode='$direct_customer' order by id DESC")->row_array()['remarks'];
		} else {
			$remarks = $this->db->query("SELECT remarks FROM `tbl_lead_generation` where customer_acode='$customer' order by id DESC")->row_array()['remarks'];
		}
		echo $remarks;
	}
