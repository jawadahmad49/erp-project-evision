...
class User extends CI_Controller
{
	...
	public function index()
	{
		...
		// $data['location'] =$this->db->query("select sp_name from tbl_sales_point where $")->row_array();
		$data["filter"] = '';
		#----load view----------#
		$data["title"] = "Manage User";
		$this->load->view("app/user/manage_users", $data);
	}
	...
	public function role($id)
	{
		...
		// APP
		$where = array('sts' => 'Active', 'pageid >' => 1040, 'pageid <=' => 1070);
		$data['app_config'] = $this->mod_common->select_array_records($table, '*', $where);

		$where = array('sts' => 'Active', 'pageid >' => 1070, 'pageid <' => 1085);
		$data['app_order_management'] = $this->mod_common->select_array_records($table, '*', $where);

		$where = array('sts' => 'Active', 'pageid >' => 1085, 'pageid <' => 1095);
		...
		$where = array('sts' => 'Active', 'pageid >' => 1199, 'pageid <' => 1210);
		$data['notification'] = $this->mod_common->select_array_records($table, '*', $where);

		$where = array('sts' => 'Active', 'pageid >' => 1160, 'pageid <' => 1179);
		$data['reports'] = $this->mod_common->select_array_records($table, '*', $where);
		// APP
		$where = array('sts' => 'Active', 'pageid >' => 1105, 'pageid <' => 1110);
		$data['feedback'] = $this->mod_common->select_array_records($table, '*', $where);
		...
		$data["id"] = $id;
		$this->load->view("app/user/user_rights", $data);
	}
	...
	{
		$data['location'] = $this->db->query("select * from tbl_sales_point")->result_array();
		...
		$data["title"] = "Add User";
		$this->load->view("app/user/add_user", $data);
	}
	...
	public function add()
	{
		...
		if ($this->input->server('REQUEST_METHOD') == 'POST') {
			...
			if ($this->mod_user->get_by_title($udata['loginid'])) {
				...
				redirect(SURL . 'app/User/add_user');
				exit();
			}
	...
	public function update()
	{
		if ($this->input->server('REQUEST_METHOD') == 'POST') {
			...
			if ($this->mod_user->edit_by_title($cdata['loginid'], $id)) {
				$this->session->set_flashdata('err_message', 'Login Id Already Exist.');
				redirect(SURL . 'app/User/edit/' . $id);
				exit();
			}
