...
class Brand extends CI_Controller
{
	...
	public function index()
	{
		...
		$this->load->view("app/brand/manage_brandes", $data);
	}

	public function add_brand()
	{
		$login_user = $this->session->userdata('id');
		$role = $this->db->query("select * from tbl_user_rights where uid = '$login_user' and pageid = '1044' limit 1")->row_array();
		if ($role['add'] != 1) {
			$this->session->set_flashdata('err_message', 'You have no authority to Complete this task .');
			redirect(SURL . 'app/Brand/index/');
		}
		...
		$this->load->view("app/brand/add_brand", $data);
	}
	public function add()
	{
		...
		if ($this->input->server('REQUEST_METHOD') == 'POST') {
			...
			if ($this->mod_brand->get_by_title($udata['brand_name'])) {
				...
				redirect(SURL . 'app/Brand/add_brand');
				exit();
			}

			$udata['status'] = $this->input->post('status');
			$udata['swap_good'] = $this->input->post('swap_good');
			$udata['swap_average'] = $this->input->post('swap_average');
			$udata['created_by'] = 1;
	...
	public function edit($id)
	{
		...
		if ($id) {
			...
			//pme($data['country']);
			$this->load->view("app/brand/edit", $data);
		}
	}
	...
	public function update()
	{
		if ($this->input->server('REQUEST_METHOD') == 'POST') {
			...
			if ($this->mod_brand->edit_by_title($mdata['brand_name'], $_POST['id'])) {
				$this->session->set_flashdata('err_message', 'Name Already Exist.');
				redirect(SURL . 'app/Brand/edit/' . $_POST['id']);
				exit();
			}
			$mdata['status'] = $_POST['status'];

			$mdata['swap_good'] = $this->input->post('swap_good');
			$mdata['swap_average'] = $this->input->post('swap_average');
			$id = $_POST['id'];
			$where = "brand_id='$id'";
