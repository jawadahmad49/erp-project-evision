...
class Return_order extends CI_Controller
{
	...
	public function index()
	{
		...
		// 	$data['from_date'] = $from_date = date("Y-m-d", strtotime($_POST['from']));
		// 	$data['to_date'] = $to_date = date("Y-m-d", strtotime($_POST['to']));
		// }
		$data["filter"] = '';
		#----load view----------#
		$data["title"] = "Cylinder Return";
		$this->load->view("app/Return_order/manage", $data);
	}
	public function your_ajax_endpoint()
	{
		...
		$this->db->from('tbl_cylinder_return');
		$this->db->where('date >=', $from_date);
		$this->db->where('date <=', $to_date);

		if (!empty($searchValue)) {
			$this->db->group_start();
			$this->db->like('order_id', $searchValue);
			$this->db->or_like('date', $searchValue);
			$this->db->or_like('total_qty', $searchValue);
			$this->db->or_like('total_amount', $searchValue);
			$this->db->group_end();
		}

		if ($sale_point_ids) {
			$this->db->where_in('sale_point_id', $sale_point_ids);
		}
		...
		foreach ($results as $value) {
			...
			$action_buttons = '<a class="green" href="' . SURL . 'app/Return_order/edit/' . $id . '">
                  <i class="ace-icon fa fa-pencil bigger-130"></i>
               </a>
               <a id="bootbox-confirm" href="javascript:void(0)" class="red" onClick="confirmDelete(\'' . SURL . 'app/Return_order/delete/' . $value['id'] . '\');">
	...
	public function get_items()
	{
		...
		$item_list = $this->db->query("SELECT materialcode,itemname FROM `tblmaterial_coding` where materialcode in (select materialcode from tbl_place_order_detail where order_id='$order' and security_charges > 0 and type='New')")->result_array(); ?>
	...
	{
		$login_user = $this->session->userdata('id');
		$role = $this->db->query("SELECT * FROM tbl_user_rights WHERE uid = ? AND pageid = '1071' LIMIT 1", [$login_user])->row_array();
		if (empty($role) || $role['add'] != 1) {
		...
		$data['sale_point_id'] = $this->db->query("SELECT sale_point_id FROM tbl_place_order where deliveryStatus='Delivered'")->row_array()['sale_point_id'];
		$data["filter"] = '';
		#----load view----------#
		$data["title"] = "Cylinder Return";

		$this->load->view("app/Return_order/add", $data);
	}
	public function edit($id)
	{
		$login_user = $this->session->userdata('id');
		$role = $this->db->query("SELECT * FROM tbl_user_rights WHERE uid = ? AND pageid = '1071' LIMIT 1", [$login_user])->row_array();
		if (empty($role) || $role['edit'] != 1) {
			$this->session->set_flashdata('err_message', 'You have no authority to complete this task.');
			redirect(SURL . 'app/Trip_coding/index/');
		}
		$data['return'] = $return = $this->db->query("SELECT * from tbl_cylinder_return where id='$id'")->row_array();
		$where_location = "WHERE sale_point_id IN ('" . $return['sale_point_id'] . "')";
		$data['salepoint'] = $this->db->query("SELECT * from tbl_sales_point $where_location")->result_array();
		$data['sale_point_id'] = $this->db->query("SELECT sale_point_id FROM tbl_place_order where id='" . $return['order_id'] . "'")->row_array()['sale_point_id'];
		$data['return_d'] =  $this->db->query("SELECT * from tbl_cylinder_return_detail where master_id='$id'")->result_array();
	...
	public function submit()
	{
		try {
			if ($this->input->server('REQUEST_METHOD') == 'POST') {
				...
				$sale_point_id = $this->session->userdata('location');
				$today = date('Y-m-d');

				// Collect main order data from the form
				$udata['date'] = $this->input->post('date');
				$udata['order_id'] = $this->input->post('order');
				$udata['total_qty'] = $this->input->post('total_qty');
				$udata['total_amount'] = $this->input->post('total_amount');
				$udata['sale_point_id'] = $sale_point_id;
				...
				if (empty($this->input->post("edit"))) {
					// Adding new record
					$udata['Created_date'] = $today;
					$udata['created_by'] = $user_id;
					$res = $this->mod_common->insert_into_table("tbl_cylinder_return", $udata);
				} else {
					// Updating an existing record
					$udata['modified_date'] = $today;
					$udata['modified_by'] = $user_id;
					$res = $this->input->post("edit");
					$this->mod_common->update_table("tbl_cylinder_return", array("id" => $res), $udata);
					// Optionally delete previous related data before updating
					$this->db->query("DELETE from tbl_cylinder_return_detail where master_id='$res'");
				}

				// Get the array data for materials
				$material_codes = $this->input->post('material_code');
				$qty = $this->input->post('qty');
				$security_charges = $this->input->post('security_charges');
				// Loop through each material and insert/update
				foreach ($material_codes as $index => $material_code) {
					$arrayy = array(
						"master_id" => $res,  // Link to the main order record
						"order_id" => $this->input->post('order'),  // Link to the main order record
						"materialcode" => $material_code,
						"quantity" => isset($qty[$index]) ? $qty[$index] : 0,
						"security_charges" => isset($security_charges[$index]) ? $security_charges[$index] : 0,
						"sale_point_id" => $sale_point_id,
					);
					// Insert into material-specific table
					$this->mod_common->insert_into_table("tbl_cylinder_return_detail", $arrayy);
				}
	...
	public function get_orders()
	{
		...
		$trip_orders = $this->db->query("SELECT GROUP_CONCAT(order_id) as order_ids FROM tbl_cylinder_return")->row_array();
		$excluded_order_ids = [];
		...
		if (!empty($order_id)) {
			...
			$query = "SELECT * 
				...
				AND id NOT IN ('$excluded_ids_str') AND id IN  (select order_id from tbl_place_order_detail where security_charges > 0 and type='New')
				ORDER BY id DESC";
		}
...
<?php }
	...
	public function get_item_detail()
	{
		...
		$order_d = $this->db->query("SELECT quantity,security_charges FROM tbl_place_order_detail WHERE order_id = '$order_id' and materialcode='$materialcode'")->row_array();
		$quantity = $order_d['quantity'];
		$security_charges = $order_d['security_charges'];
		$amount = $security_charges * $quantity;
		$item_detail = $this->db->query("SELECT itemname, catcode FROM tblmaterial_coding WHERE materialcode = '$materialcode'")->row_array();
		...
		// Return the response
		echo $catname . "|" . $security_charges . "|" . $options . "|" . $quantity . "|" . $amount;
	}
	...
	public function delete($id)
	{
		...
		$this->db->where('id', $id);
		$this->db->delete('tbl_cylinder_return');
		$this->db->where('master_id', $id);
		$this->db->delete('tbl_cylinder_return_detail');
		$this->db->trans_complete();
		...
		redirect(SURL . 'app/Return_order/');
	}
