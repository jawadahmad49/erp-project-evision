...
class City extends CI_Controller {
	...
	public function index()
	{
		...
		$this->load->view("app/city/manage_cities",$data);
	...
	public function add_city()
	{
		$login_user=$this->session->userdata('id');
	    	$role = $this->db->query("select * from tbl_user_rights where uid = '$login_user' and pageid = '1043' limit 1")->row_array();
		if ($role['add']!=1) {
			$this->session->set_flashdata('err_message', 'You have no authority to Complete this task .');
			redirect(SURL . 'app/City/index/');
			}
    	$table='tbl_country';       
        $data['country_list'] = $this->mod_common->get_all_records($table,"*");    			
		$this->load->view("app/city/add_city",$data);
	}
	...
	public function add(){
		...
		if($this->input->server('REQUEST_METHOD') == 'POST'){
			...
			if ($this->mod_city->get_by_title($udata['city_name'])) {
				...
				redirect(SURL . 'app/City/add_city');
				exit();
			}
	...
	public function edit($id){
		...
		if($id){
			...
			//pme($data['country']);
			$this->load->view("app/city/edit", $data);
		}
	}
		...
		if($this->input->server('REQUEST_METHOD') == 'POST'){
			...
			#----check name already exist---------#
				if ($this->mod_city->edit_by_title($cdata['city_name'],$id)) {
					$this->session->set_flashdata('err_message', 'Name Already Exist.');
					redirect(SURL . 'app/City/edit/'.$id);
					exit();
				}
