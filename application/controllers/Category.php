...
class Category extends CI_Controller {
	...
	public function index()
	{
		...
		$this->load->view($this->session->userdata('language')."/category_coding/manage_category",$data);
	}

	public function add_category()
	{    
		$login_user=$this->session->userdata('id');
	    	$role = $this->db->query("select * from tbl_user_rights where uid = '$login_user' and pageid = '7' limit 1")->row_array();
		if ($role['add']!=1) {
			$this->session->set_flashdata('err_message', 'You have no authority to Complete this task .');
			redirect(SURL . 'Category/index/');
			}
		$table='tblclass';       
        $data['class_list'] = $this->mod_common->get_all_records($table,"*");
		$this->load->view($this->session->userdata('language')."/category_coding/add_category",$data);
	}
    ...
    public function edit($id){
		...
		$data['class_list'] = $this->mod_common->get_all_records($table,"*");
		//pme($data['country']);
		$this->load->view($this->session->userdata('language')."/category_coding/edit", $data);
	}
