...
class Basestockreport extends CI_Controller {
	...
	public function __construct() {
        ...
        $this->load->model(array(
            "mod_basestockreport","mod_common"
        ));
	...
	public function index()
	{
		...
		$data['basestock_report_list'] = $this->mod_common->get_all_records($table,"*");
 
		$data["filter"] = '';
		#----load view----------#
		$data["title"] = "Base Stock Report";	
		$this->load->view($this->session->userdata('language')."/basestock_report/search",$data);
	}
	...
	public function details()
	{
		...
		if($this->input->server('REQUEST_METHOD') == 'POST'){
			...
			$data['report']=  $this->mod_basestockreport->get_details($this->input->post());
			//pm($data['report']); exit;
        ...
        $data['company'] = $this->mod_common->get_all_records($table,"*");
			if ($data['report']) {
	            ...
	            $data["title"] = "Base Stock Report";
	            $this->load->view($this->session->userdata('language')."/basestock_report/detail",$data);
	        } else {
	            $this->session->set_flashdata('err_message', 'No Record Found.');
	            redirect(SURL . 'Basestockreport/');
	        }
	    }else{
	        //$data["filter"] = 'add';
	        $data["title"] = "Base Stock Report";    		
	...
	public function add()
	{
		if($this->input->server('REQUEST_METHOD') == 'POST'){
			...
			if ($add) {
			 	$this->session->set_flashdata('ok_message', 'You have succesfully added.');
	            redirect(SURL . 'basestock_report/');
	        } else {
        ...
        $data["filter"] = 'add';
        $data["title"] = "Add Base Stock Report";    			
		$this->load->view($this->session->userdata('language')."/basestock_report/add",$data);
	}
	...
	public function edit($id){
		if($id){
	        ...
	        $data["filter"] = 'edit';
        	$data["title"] = "Update Base Stock Report";
			$this->load->view($this->session->userdata('language')."/basestock_report/add", $data);
		}
		/* Update Data */
		if($this->input->server('REQUEST_METHOD') == 'POST'){
