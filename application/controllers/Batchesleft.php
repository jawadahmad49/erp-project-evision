...
class Batchesleft extends CI_Controller {
	...
	ini_set('memory_limit','2048M');
        ...
        $this->load->view($this->session->userdata('language')."/Batchesleft/search",$data);       	
	}
	...
	{							
		...
		$data['batchesleft'] = $this->db->query("select * from close_profit where month='$mnth' and year='".$this->input->post("year")."'")->result_array()[0]['batches_left'];
		$data['batchesleft'] = explode("-",$data['batchesleft']);

		$data['sales'] = $this->db->query("select tbl_issue_goods_detail.purchase_batch_no from tbl_issue_goods_detail inner join tblmaterial_coding on tbl_issue_goods_detail.itemid=tblmaterial_coding.materialcode inner join tbl_issue_goods on tbl_issue_goods.issuenos=tbl_issue_goods_detail.ig_detail_id where tblmaterial_coding.catcode='1' and tbl_issue_goods.type='Fill' and tbl_issue_goods.issuedate between '$from' and '$to' ")->result_array();
		//pm($data['sales']);

		$record="";
		foreach ($data['sales'] as $key => $value) {
			$explodedresult = explode(",", $value['purchase_batch_no']);

			foreach ($explodedresult as $key => $value123) {
		...
		$data['sales'] = array_unique($record);
		//pm($data['sales']);

		$this->load->view($this->session->userdata('language')."/Batchesleft/detail_report",$data);
