...
class Order_feedback extends CI_Controller
{
	...
	public function index()
	{
		...
		$login_user = $this->session->userdata('id');
		$data['bookorder_list'] = $this->mod_bookorder->manage_bookorder($from_date, $to_date, $sale_point_id, 'Delivered');

		$data["filter"] = '';
		#----load view----------#
		$data["title"] = "Manage Order Feedback";
		$this->load->view("en/order_feedback/manage_delivered_order", $data);
	}
	...
	public function add_delivered_order()
	{
		...
		$data["title"] = "Order Feedback";
		$this->load->view("en/order_feedback/add_delivered_order", $data);
	}
	...
	public function edit($id)
	{
		if ($id) {
			...
			#----load view----------#
			$data["title"] = "Update Order Feedback";
			$this->load->view("en/order_feedback/edit", $data);
		}
	}

	public function update()
	{
		//adding data of feedback columns into table
		...
		if ($this->input->server('REQUEST_METHOD') == 'POST') {
			...
			$array_feedback = array(
				...
				"feedback_emoji" => $this->input->post('feedback_input'),
				"feedback_by" => $login_user,
				"feedback_date_time" => date('Y-m-d h:i:sa'),
			...
			$where = "id ='$id'";
			$table='tbl_orderbooking';
			$add_bookorder=$this->mod_common->update_table($table,$where,$array_feedback); 
	...
	public function detail($id)
	{
		...
		if ($id) {
			...
			$this->load->view("en/order_feedback/single", $data);
		}
	}
...
?>
	...
	public function get_branch()
	{
		...
		$login_user = $this->session->userdata('id');
		$sale_point_id = $this->db->query("  location from tbl_admin where id='$login_user'")->row_array()['location'];
		if ($sale_point_id == '0') {
			$where_sale_point_id = "";
		} else {
			$where_sale_point_id = "and sale_point_id='$sale_point_id'";
