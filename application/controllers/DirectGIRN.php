...
class DirectGIRN extends CI_Controller {
	...
	{	
		...
		$data['directgirn_list'] = $this->mod_girndirect->manage_directgirn($from_date,$to_date,$sale_point_id);
		...
		$this->load->view($this->session->userdata('language')."/direct_girn/direct_girn",$data);
	}
	...
	public function add_direct_girn()
	{
		...
		if ($role['add']!=1) {
			...
			redirect(SURL . 'DirectGIRN/index/');
			}
		    $login_user=$this->session->userdata('id');
            $sale_point_id = $this->db->query("select location from tbl_admin where id='$login_user'")->row_array()['location'];
             if ($sale_point_id=='0') {
  ...
  //       $data['item_list'] = $this->mod_common->get_all_records($table,"*");
		...
		$this->load->view($this->session->userdata('language')."/direct_girn/add_direct_girn",$data);
	}
	...
	public function add(){
		...
		if($this->input->server('REQUEST_METHOD') == 'POST'){
			...
			if(!empty($last_date))
			{
				...
				redirect(SURL . 'DirectGIRN/add_direct_girn');
			}
 ...
 //pm($data['single_edit']);
		...
		///////////////////////////////////////////////////////////////////////////////////////////////		
		////if cash then get cash balance , if bank then get bank balance/////////////////////////////
        ...
        $data['trans_data']= $this->mod_girndirect->select_single_trans($id_tars);
      ...
      //pm($data['trans_data']);
        ...
        $data['trans_detail']= $this->mod_girndirect->select_single_trans_detail($id_tars);

        // pm($data['trans_detail']);

        

        //pm($data['trans_detail']);

        $data['trans_bank']= $this->mod_girndirect->select_single_trans_bank($where_cat_id);
		...
		$data["edit_id"] = $id;
		#----load view----------#
		$data["title"] = "Update Purchase Filled";
		$this->load->view($this->session->userdata('language')."/direct_girn/edit",$data);
		}
	}
...
// exit;
	...
	public function get_cylinder($vendor_id){
        ...
        $where_cat_id = array('catcode' => 1);
        $data['item_list_new']= $this->mod_girndirect->get_brand_item($where_cat_id,$brand_array);
        ?>
 ...
 //       // $repost = $this->mod_girndirect->repost_purchase($parentid);
 ...
 //            echo '1';
	...
	public function detail($id){
		...
		$this->load->view($this->session->userdata('language')."/direct_girn/single",$data);
		}
	}
  ...
  // pm(	$data['final_bal']);
		...
		$this->load->view($this->session->userdata('language')."/direct_girn/single_small",$data);
		}
	}
}
