...
class Swap_cyl_recv_report extends CI_Controller {
	...
	public function index()
	{
		...
		$data['items']=$this->db->query("select * from tblmaterial_coding where catcode='7'")->result_array();
		$data["filter"] = '';
		#----load view----------#
		$data["title"] = "Swap Cylinder Receive Report";	
		$this->load->view($this->session->userdata('language')."/Swap_cyl_recv_report/search",$data);
	...
	{																		
		...
		// pm($item);exit();
		if ($item!='All') {$where_item="and tbl_swap_recv_detail.itemid='$item'";}else{$where_item='';}
			


			$data['report']=$this->db->query ("SELECT tbl_swap_recv.*,tbl_swap_recv_detail.* From tbl_swap_recv INNER JOIN tbl_swap_recv_detail ON tbl_swap_recv.irnos = tbl_swap_recv_detail.irnos where  tbl_swap_recv.type='SCR' and  tbl_swap_recv.irdate between '$from_date' and '$to_date' $where_item")->result_array();




// pm($data['report']);exit();
			// $data['report'] where tbl_swap_recv.irdate between '$from_date' and '$to_date' and=$this->db->query("select type='SCR' from tbl_swap_recv where" )->result_array();
			...
			$new_date['customer']=$this->input->post('scode');
			...
			if ($data['report']) {
	            ...
	            $this->load->view($this->session->userdata('language')."/Swap_cyl_recv_report/single",$data);
	        } else {
	            $this->session->set_flashdata('err_message', 'No Record Found.');
	            redirect(SURL . 'Swap_cyl_recv_report');
	        }
	}
