...
class Return_to_plant extends CI_Controller {

	public function __construct() {
        parent::__construct();

        $this->load->model(array(
            "mod_customer","mod_common","Mod_return_to_plant","mod_stockreport","mod_customerledger","mod_bank"
        ));
	...
	public function index()
	{
		...
		$data['swap_sale_list'] = $this->Mod_return_to_plant->manage_swap_sale($from_date,$to_date,$sale_point_id);
		
		$data["filter"] = '';
		#----load view----------#
		$data["title"] = "Manage Swap Cylinder Return to Plant";
		$this->load->view($this->session->userdata('language')."/Return_to_plant/manage_swap_sale",$data);
	}
		...
		public function add_swap_sale()
	{
		...
		if ($role['add']!=1) {
			...
			redirect(SURL . 'Return_to_plant/index/');
			}
		$login_user=$this->session->userdata('id');
        $sale_point_id = $this->db->query("select location from tbl_admin where id='$login_user'")->row_array()['location'];
        $sale_point_id = $this->db->query("select location from tbl_admin where id='$login_user'")->row_array()['location'];
       ...
       $data['customer_list'] =$this->db->query("select * from tblacode  where atype='Child' and general in('$customer_code','$vendor_code','$bank_code','$expense_code','$empty_stock_code','$empty_sale_code','$security_code') or left(acode,6)='$exp_code' or left(acode,6)='200100' or general in ('1002003000','2006001000') or tblacode.acode in ('$cash_code','$sale_point_id','$tax_pay','$tax_receive','$sales_code','$stock_code','$cost_of_goods_code')")->result_array();
		...
		$this->load->view($this->session->userdata('language')."/Return_to_plant/add_swap_sale",$data);
	}
	public function add(){
		...
		if($this->input->server('REQUEST_METHOD') == 'POST'){
			...
			if(!empty($last_date))
			{
				...
				redirect(SURL . 'Return_to_plant/add_swap_sale');
			}

		 //echo "<pre>";print_r($this->input->post());exit;
			$add=  $this->Mod_return_to_plant->add_swap_sale($this->input->post());
	...
	public function edit($id){
       ...
       $data['customer_list'] =$this->db->query("select * from tblacode  where atype='Child' and general in('$customer_code','$vendor_code','$bank_code','$expense_code','$empty_stock_code','$empty_sale_code','$security_code') or left(acode,6)='$exp_code' or left(acode,6)='200100' or general in ('1002003000','2006001000') or tblacode.acode in ('$cash_code','$sale_point_id','$tax_pay','$tax_receive','$sales_code','$stock_code','$cost_of_goods_code')")->result_array();
		...
		$data['edit_list'] = $this->Mod_return_to_plant->edit_swap_sale($id);
	 ...
	 // 	}
		...
		$data["filter"] = '';
		#----load view----------#
		$data["title"] = "Update Swap Cylinder Return to Plant";
		$this->load->view($this->session->userdata('language')."/Return_to_plant/edit",$data);
		}
	}
	...
	public function makenew($id){
		...
		$data["id"] = $id;
		#----load view----------#
		$data["title"] = "Update Return To Appliances";
		$this->load->view($this->session->userdata('language')."/Return_to_plant/add_sale_lpg",$data);
		}
	}
	...
	public function update(){
		if($this->input->server('REQUEST_METHOD') == 'POST'){
			...
			$add_salelpg=  $this->Mod_return_to_plant->update_swap_sale($this->input->post());
...
function record_delete()
	...
	public function detail($id){
		...
		$this->load->view($this->session->userdata('language')."/Return_to_plant/single",$data);
		}
	}
	...
	public function detail_salestax($id){
		...
		#----load view----------#
		$data["title"] = "Customer Invoice";
		$this->load->view($this->session->userdata('language')."/Return_to_plant/single_salestax",$data);
		}
	}
...
$issuedate= $data['single_edit']['issuedate'];
		...
		$this->load->view($this->session->userdata('language')."/Return_to_plant/single_small",$data);
		}
	}
