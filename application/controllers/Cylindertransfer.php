...
class Cylindertransfer extends CI_Controller {
	...
	public function index()
	{
		...
		$this->db->where($where);
		$query = $this->db->get();
		$data['paymentreceipt_list'] = $query->result_array();
		
	
		$data["filter"] = ''; 
		#----load view----------#
		$data["title"] = "Manage Return Cylinders";	
		$this->load->view($this->session->userdata('language')."/cylindertransfer/manage_paymentreceipt",$data);
	}
 ...
 if($to_date==''){$to_date=date('Y-m-d');}
		...
		$this->load->view($this->session->userdata('language')."/cylindertransfer/detail",$data);
	...
	public function detail($id){
		...
		if($id){
			...
			$this->load->view($this->session->userdata('language')."/cylindertransfer/single",$data);
		}
		else{
			redirect(SURL.'Cylindertransfer');
		}
	}
	...
	public function add()
	{
		...
		if($this->input->server('REQUEST_METHOD') == 'POST'){
			...
			$add=  $this->Mod_securityreceipt->addrecord($this->input->post());
        ...
        $data["filter"] = 'add';
        $data["title"] = "Add Cylinder Transfer";    			
		$this->load->view($this->session->userdata('language')."/cylindertransfer/add",$data);
	}
	...
	public function edit($id){ 
		...
		if($this->input->server('REQUEST_METHOD') == 'POST'){
			...
			redirect(SURL . 'Cylindertransfer/edit/'.$id);
			}
			$this->db->trans_start();
