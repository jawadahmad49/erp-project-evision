...
class Stock_Report extends CI_Controller {
	...
	{ 
		// pm($this->input->post());
		$data['itemss'] = $this->db->query("select * from tblmaterial_coding where catcode!='1'")->result_array();
		...
		#----load view----------#
		$data["title"] = "Stock Report Appliances";	
		$this->load->view($this->session->userdata('language')."/StockReport/search",$data);
	}
	...
	public function report()
	{
		//pm($this->input->post());
		    ...
		    $data['item_pdf']=$this->input->post("item");
		    if($item!='All'){ $where= "and tblmaterial_coding.materialcode='$item'"; }else{ $where= ""; }
          	$data['report'] = $this->db->query("select * from tblmaterial_coding where catcode!='1' $where")->result_array();
            $data["title"]="Stock&nbsp;Report&nbsp;Appliances";
		 //pm($data['report']);
			$table='tbl_company';       
       		$data['company'] = $this->mod_common->get_all_records($table,"*");
			if ($data['report']) {
			$this->load->view($this->session->userdata('language')."/StockReport/single",$data);
	        } else {
    ...
    if($this->input->server('REQUEST_METHOD') == 'POST'){
		    ...
		    $to_date=$data['to_date']=$this->input->post("to_date");
		    $location=$data['location']=$this->input->post("location");
		    //$item = $data['item'] = $this->input->post("item");
		    $data['item']=$item=$this->input->post("item");
		   //echo $data['item'];exit;
		    if($item!='All'){ $where= "and tblmaterial_coding.materialcode='$item'"; }else{ $where= ""; }
		    //echo "select * from tblmaterial_coding where catcode!='1' $where";exit;
          	$data['report'] = $this->db->query("select * from tblmaterial_coding where catcode!='1' $where")->result_array();
          	//pm($data['report']);exit;
            $data["title"]="Stock&nbsp;Report&nbsp;Appliances";
		 //pm($data['report']);
	  ...
	  //pm($data);
		...
		$this->load->view($this->session->userdata('language')."/StockReport/pdffile",$data);
