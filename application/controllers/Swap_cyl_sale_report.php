...
class Swap_cyl_sale_report extends CI_Controller {
	...
	public function index()
	{
		...
		$data["filter"] = '';
		#----load view----------#
		$data["title"] = "Swap Cylinder Sale Report";	
		$this->load->view($this->session->userdata('language')."/Swap_cyl_sale_report/search",$data);
	...
	{																		
		...
		if ($item!='All') {$where_item="and tbl_swap_recv_detail.itemid='$item'";}else{$where_item='';}
			...
			$data['report']=$this->db->query ("SELECT tbl_swap_recv.*,tbl_swap_recv_detail.* From tbl_swap_recv INNER JOIN tbl_swap_recv_detail ON tbl_swap_recv.irnos = tbl_swap_recv_detail.irnos where  tbl_swap_recv.type='SCS' and  tbl_swap_recv.irdate between '$from_date' and '$to_date' $where_item")->result_array();
			...
			if ($data['report']) {
	            ...
	            $this->load->view($this->session->userdata('language')."/Swap_cyl_sale_report/single",$data);
	        } else {
	            $this->session->set_flashdata('err_message', 'No Record Found.');
	            redirect(SURL . 'Swap_cyl_sale_report');
	        }
	}
