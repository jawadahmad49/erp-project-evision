...
class Mod_dar extends CI_Model {
...
$queryj = $this->db->query($sqlj);
	if($queryj->num_rows()>0){
		foreach($queryj->result_array() as $key => $value) {
			$sqljj="SELECT itemid , wrate ,returns ,qty , returns, sprice,tblmaterial_coding.itemname FROM `tbl_issue_goods_detail` INNER JOIN `tblmaterial_coding` ON `tbl_issue_goods_detail`.`itemid` = `tblmaterial_coding`.`materialcode` where  `ig_detail_id`='".$value['issuenos']."'
...
$queryj = $this->db->query($sqlj);
	if($queryj->num_rows()>0){
		foreach($queryj->result_array() as $key => $value) {
			$sqljj="SELECT itemid , wrate  ,qty , total_amount as sprice,tblmaterial_coding.itemname FROM `tbl_issue_return_detail` INNER JOIN `tblmaterial_coding` 
...
return $dataj;
   ...
   $queryj = $this->db->query($sqlj);
    ...
    $sqljj="SELECT itemid , (rate +vat_amount) as rate ,quantity , ex_vat_amount, tblmaterial_coding.itemname FROM `tbl_goodsreceiving_detail`
