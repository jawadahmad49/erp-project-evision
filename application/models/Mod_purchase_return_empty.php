...
class Mod_purchase_return_empty extends CI_Model {
	...
	public function add_purchase_return_empty($data){
...
foreach ($data['item'] as $key => $value) {
	...
	$qty =  $data['qty'][$key];
			...
			//$goodsid=$goodsid."-G";
			$goodsids=$sale_point_id."-PRE-".$trans_id;
		...
		/////////////////////////// here is code//////////////////
		 ...
		 $nar='Purchase Return Filled #:'.$trans_id.',  '.$items_detail_m.'('.$remarks.')';
			...
			values
			('$goodsids' , 'SV' , '$netamount' , '$netamount' ,'No' ,'No' ,'$user','PRE' ,'$receiptdate','$sale_point_id','$trans_id')";
		    ...
		    $sr++;
				$stock_code=$fix_code['empty_stock_code'];
			   ...
			   values('$goodsids','$sr','$vendorcode','$vendorname','$amount','0','$nar','SV','PRE','$receiptdate','$sale_point_id','$trans_id','$master_id')";
				...
				values('$goodsids','$sr','$stock_code','$stock_name','0','$amount','$nar','SV','PRE','$receiptdate','$sale_point_id','$trans_id','$master_id')";
	...
	public function manage_return_empty($from,$to,$sale_point_id){
		...
		$this->db->where('tbl_issue_goods_detail.type','Empty');
		 ...
		 $nar='Purchase Return Filled #:'.$trans_id.',  '.$items_detail_m.'('.$remarks.')';
		    ...
		    $sr++;
				$stock_code=$fix_code['empty_stock_code'];
				 ...
				 $sr++;
		       $queryd = "INSERT into `tbltrans_detail` (vno,srno,acode,aname,damount,camount,remarks,vtype,svtype,vdate,sale_point_id,trans_id,ig_detail_id) 
			   values('$goodsids','$sr','$vendorcode','$vendorname','$netamount','0','$nar','SV','PRE','$receiptdate','$sale_point_id','$trans_id','$master_id')";
				$this->db->query($queryd);
				$debit+=$netamount;
				
				 $sr++;
			    $resultdd ="INSERT INTO `tbltrans_detail`(vno,srno,acode,aname,damount,camount,remarks,vtype,svtype,vdate,sale_point_id,trans_id,ig_detail_id)
				values('$goodsids','$sr','$stock_code','$stock_name','0','$netamount','$nar','SV','PRE','$receiptdate','$sale_point_id','$trans_id','$master_id')";
				$this->db->query($resultdd);
				$credit+=$netamount;
