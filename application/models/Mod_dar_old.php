...
class Mod_dar_old extends CI_Model {
...
$queryj = $this->db->query($sqlj);
	if($queryj->num_rows()>0){
		foreach($queryj->result_array() as $key => $value) {
			$sqljj="SELECT itemid , wrate ,returns ,qty , sprice FROM `tbl_issue_goods_detail` INNER JOIN `tblmaterial_coding` ON `tbl_issue_goods_detail`.`itemid` = `tblmaterial_coding`.`materialcode` where  `ig_detail_id`='".$value['issuenos']."'
...
$queryj = $this->db->query($sqlj);
	if($queryj->num_rows()>0){
		foreach($queryj->result_array() as $key => $value) {
			$sqljj="SELECT itemid , wrate  ,qty , total_amount as sprice FROM `tbl_issue_return_detail` INNER JOIN `tblmaterial_coding` 
...
$condj3 = "";
  ...
  $sqlj="  select * from tbltrans_detail where acode in($bank_codes  '2003013001') and vdate= '$fdate' and damount='0'";
$queryj = $this->db->query($sqlj);
	if($queryj->num_rows()>0){
		foreach($queryj->result_array() as $key => $value) {
...
$condj3 = "";
  ...
  $sqlj="  select * from tbltrans_detail where acode in($bank_codes  '2003013001') and vdate= '$fdate' and camount='0'";
$queryj = $this->db->query($sqlj);
	if($queryj->num_rows()>0){
		foreach($queryj->result_array() as $key => $value) {
			$sqljj="SELECT * FROM `tbltrans_detail` INNER JOIN `tblacode` 
...
return $dataj;
   ...
   $queryj = $this->db->query($sqlj);
    ...
    $sqljj="SELECT itemid , (rate +vat_amount) as rate ,quantity , ex_vat_amount FROM `tbl_goodsreceiving_detail`
