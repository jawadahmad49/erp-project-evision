...
class Mod_swap_receive extends CI_Model {
	...
	public function add_swap_receive($data){
		//pm($data['makenew']);
	...
	public function multipleitems_againstid($data,$goodsid,$trans_id,$table){
		...
		/////////////////////////// here is code//////////////////
			...
			$goodsidt=$sale_point_id."-SCR-".$trans_id;
		  ...
		  if($query->num_rows()!=0)
		  {
			...
			$nar='Swap Cylinder Receive against #: '.$trans_id.', '.$items_detail_m.','.$remarks;
			$branch_code=$data['scode'];
			...
			values
			('$goodsidt' , 'SV' , '$netamount' , '$netamount' ,'No' ,'No' ,'$user','SCR' ,'$receiptdate','$sale_point_id','$trans_id')";
			...
			values('$goodsidt','$sr','$other_cylinder_stock','$stock_name','$netamount','0','$nar','SCR','','$receiptdate','$sale_point_id','$trans_id','$branch_code','$master_id')";
			$this->db->query($resultdd);
			$debit+=$netamount;
			 $sr++;				
			$resultdd ="INSERT INTO `tbltrans_detail`(vno,srno,acode,aname,damount,camount,remarks,vtype,svtype,vdate,sale_point_id,trans_id,scode,ig_detail_id)
			values('$goodsidt','$sr','$vendorcode','$stock_name','0','$netamount','$nar','SCR','','$receiptdate','$sale_point_id','$trans_id','$branch_code','$master_id')";
			$this->db->query($resultdd);
			$credit+=$netamount;
	...
	public function manage_swap_receive($from,$to,$sale_point_id){
		...
		$this->db->where('tbl_swap_recv.type','SCR');
	...
	public function edit_swap_recv($id){
	...
	public function update_swap_recv($data){
		//pm($data);
		$login_user=$this->session->userdata('id');
        $sale_point_id = $this->db->query("select location from tbl_admin where id='$login_user'")->row_array()['location'];
        $this->db->trans_start();
	...
	public function updatemultiple_againstid($data,$goodsid,$table){
		...
		/////////////////////////// here is code//////////////////
		  ...
		  if($query->num_rows()!=0)
		  {
			...
			$nar_new='Swap Cylinder Receive against #: '.$trans_id.', '.$items_detail_m.','.$remarks;
				 
			$resultdd ="INSERT INTO `tbltrans_detail`(vno,srno,acode,aname,damount,camount,remarks,vtype,svtype,vdate,sale_point_id,trans_id,scode,ig_detail_id)
			values('$goodsidt','$sr','$other_cylinder_stock','$stock_name','$netamount','0','$nar_new','SCR','','$receiptdate','$sale_point_id','$trans_id','$branch_code','$master_id')";
			$this->db->query($resultdd);
			$debit+=$netamount;
			 $sr++;				
			$resultdd ="INSERT INTO `tbltrans_detail`(vno,srno,acode,aname,damount,camount,remarks,vtype,svtype,vdate,sale_point_id,trans_id,scode,ig_detail_id)
			values('$goodsidt','$sr','$vendorcode','$stock_name','0','$netamount','$nar_new','SCR','','$receiptdate','$sale_point_id','$trans_id','$branch_code','$master_id')";
			$this->db->query($resultdd);
			$credit+=$netamount;
