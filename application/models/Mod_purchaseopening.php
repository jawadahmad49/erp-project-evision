...
class Mod_purchaseopening extends CI_Model {
	...
	public function add_purchase_empty($data){
      ...
      if($trans_id==''){
		...
		$type=$data['type'][0];
		if ($type=='Empty') {
			$trans_type='purchaseempty';
		}else if ($type=='Filled') {
			$trans_type='purchasefilled';
		}else if ($type=='Other'){
			$trans_type='purchaseother';
		}
		 ...
		 // $new_nar='Payment against purchase #:'.$trans_id.'('.$remarks.')';
		  ...
		  //      $queryd = "INSERT into `tbltrans_detail` (vno,srno,acode,aname,damount,camount,remarks,vtype,svtype,vdate,sale_point_id,trans_id,ig_detail_id) 
			 //   values('$goodsids','$sr','$vendorcode','$vendorname','0','$ex_amount','$nar','PV','GP','$receiptdate','$sale_point_id','$trans_id','$master_id')";
				...
				// 	$sr++;
			 //       $queryd = "INSERT into `tbltrans_detail` (vno,srno,acode,aname,damount,camount,remarks,vtype,svtype,vdate,sale_point_id,trans_id,ig_detail_id) 
				//    values('$goodsids','$sr','$vendorcode','$vendorname','0','$taxAmount','Tax: $nar','PV','GP','$receiptdate','$sale_point_id','$trans_id','$master_id')";
				// 	$this->db->query($queryd);
				// }
...
// 				}
	...
	// 				values('$goodsids','$sr','$stock_code','$stock_name','$amount','0','$nar','PV','GP','$receiptdate','$sale_point_id','$trans_id','$master_id')";
	// 				$this->db->query($resultdd);
	// 			}
				...
				// 	$sr++;
				// 	$resultdd ="INSERT INTO `tbltrans_detail`(vno,srno,acode,aname,damount,camount,remarks,vtype,svtype,vdate,sale_point_id,trans_id,ig_detail_id)
				// 	values('$goodsids','$sr','$tax_acode','','$taxAmount','0','Tax: $nar','PV','GP','$receiptdate','$sale_point_id','$trans_id','$master_id')";
				// 	$this->db->query($resultdd);
				//  }
	 ...
	 // if($enter_amount>0){
		// // $querys="INSERT into `tbltrans_master` (vno,vtype,damount,camount,authenticate,printed,created_by,svtype,created_date,net_payment,discount,sale_point_id,trans_id)
		// // values
		// // ('$goodsids' ,'$trans_type_new' , '$enter_amount' , '$enter_amount' ,'No' ,'No' ,'$user','$trans_type_new' ,'$receiptdate','$net_payment','$discount','$sale_point_id','$trans_id')";
		// // $this->db->query($querys);
...
//}
	...
	public function manage_purchaseempty($from,$to,$sale_point_id){
		...
		$this->db->where('tbl_goodsreceiving.Purchase_type','Opening');
...
// 					$sr++;
// 			       $queryd = "INSERT into `tbltrans_detail` (vno,srno,acode,aname,damount,camount,remarks,vtype,svtype,vdate,sale_point_id,trans_id,ig_detail_id) 
// 				   values('$goodsids','$sr','$vendorcode','$vendorname','0','$gstAmt','Tax: $nar','PV','GP','$receiptdate','$sale_point_id','$trans_id','$master_id')";
// 					$this->db->query($queryd);
// 				}
...
// 					$resultdd ="INSERT INTO `tbltrans_detail`(vno,srno,acode,aname,damount,camount,remarks,vtype,svtype,vdate,sale_point_id,trans_id,ig_detail_id)
// 					values('$goodsids','$sr','$stock_code','$stock_name','$amount','0','$nar_new','PV','GP','$receiptdate','$sale_point_id','$trans_id','$master_id')";
// 					$this->db->query($resultdd);
// 				}
...
// 					$sr++;
// 					$resultdd ="INSERT INTO `tbltrans_detail`(vno,srno,acode,aname,damount,camount,remarks,vtype,svtype,vdate,sale_point_id,trans_id,ig_detail_id)
// 					values('$goodsids','$sr','$tax_acode','','$gstAmt','0','Tax: $nar','PV','GP','$receiptdate','$sale_point_id','$trans_id','$master_id')";
// 					$this->db->query($resultdd);
// 				 }
