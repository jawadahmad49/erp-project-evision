...
class Mod_swap_sale extends CI_Model {
	...
	public function multipleitems_againstid($data,$goodsid,$trans_id,$table){
		...
		/////////////////////////// here is code//////////////////
			...
			$goodsidt=$sale_point_id."-SCS-".$trans_id;
		  ...
		  if($query->num_rows()!=0)
		  {
			...
			$nar='Swap Cylinder Sale against #: '.$trans_id.', '.$items_detail_m.','.$remarks;
			$branch_code=$data['scode'];
			...
			values
			('$goodsidt' , 'SV' , '$netamount' , '$netamount' ,'No' ,'No' ,'$user','SCS' ,'$receiptdate','$sale_point_id','$trans_id')";
			...
			values('$goodsidt','$sr','$vendorcode','$stock_name','$netamount','0','$nar','SCS','','$receiptdate','$sale_point_id','$trans_id','$branch_code','$master_id')";
			$this->db->query($resultdd);
			$debit+=$netamount;
			 $sr++;				
			$resultdd ="INSERT INTO `tbltrans_detail`(vno,srno,acode,aname,damount,camount,remarks,vtype,svtype,vdate,sale_point_id,trans_id,scode,ig_detail_id)
			values('$goodsidt','$sr','$other_cylinder_stock','$stock_name','0','$netamount','$nar','SCS','','$receiptdate','$sale_point_id','$trans_id','$branch_code','$master_id')";
			$this->db->query($resultdd);
			$credit+=$netamount;
	...
	public function manage_swap_sale($from,$to,$sale_point_id){
		...
		$this->db->where('tbl_swap_recv.type','SCS');
	...
	public function updatemultiple_againstid($data,$goodsid,$table){
		...
		/////////////////////////// here is code//////////////////
		  ...
		  if($query->num_rows()!=0)
		  {
			...
			$nar_new='Swap Cylinder Sale against #: '.$trans_id.', '.$items_detail_m.','.$remarks;
