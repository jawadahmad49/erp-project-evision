...
class Mod_purchaseother extends CI_Model {
	...
	public function add_purchase_other($data){
      ...
      if($trans_id==''){
		...
		$ins_array = array(
		    ...
		    "trans_id" =>$trans_id,
		     "trans_typ" =>'purchaseother',
		      "Purchase_type" =>'purchaseother',   
		);
	...
	public function repost_purchase($goodsid){
		...
		$nar='Purchase other #:'.$goodsid.',  '.$items_detail_m.'('.$remarks.')'; 
		...
		/////////////////////////// here is code//////////////////
		 ...
		 $items_detail='';
				...
				foreach($rw as $key=>$value) {
					...
					$items_detail_m.=$value['itemname'].' ,  '.$value['quantity'].'@'.$value['rate'];
					$items_detail_m_tax.=$value['itemname'].' ,  '.$value['quantity'].'@'.$value['rate'].':';
			 
			}
		 $nar='Purchase Other #:'.$trans_id.',  '.$items_detail_m.'('.$remarks.')';
		 $nar_tax='Purchase Other #:'.$trans_id.',  '.$items_detail_m_tax.'('.$remarks.')';
	...
	$ex_amount = $netamount - $gstAmt;
				...
				$appliances_code=$fix_code['appliances_code'];
				 $sr++;
					...
					values('$goodsids','$sr','$appliances_code','$stock_name','$ex_amount','0','$nar','PV','GP','$receiptdate','$sale_point_id','$trans_id','$master_id')";
					$this->db->query($resultdd);
					$debit+=$ex_amount;
		     ...
		     //insert into transaction details debit entry
			   ...
			   values('$goodsids','$sr','$vendorcode','$vendorname','0','$ex_amount','$nar','PV','GP','$receiptdate','$sale_point_id','$trans_id','$master_id')";
				...
				if($vat_amount>0){
					$sr++;
					$resultdd ="INSERT INTO `tbltrans_detail`(vno,srno,acode,aname,damount,camount,remarks,vtype,svtype,vdate,sale_point_id,trans_id,ig_detail_id)
					values('$goodsids','$sr','$tax_acode','','$vat_amount','0','Tax $gstp % : $nar_tax','PV','GP','$receiptdate','$sale_point_id','$trans_id','$master_id')";
					$this->db->query($resultdd);
					$debit+=$vat_amount;
						$sr++;
			       $queryd = "INSERT into `tbltrans_detail` (vno,srno,acode,aname,damount,camount,remarks,vtype,svtype,vdate,sale_point_id,trans_id,ig_detail_id) 
				   values('$goodsids','$sr','$vendorcode','$vendorname','0','$vat_amount','Tax $gstp % : $nar_tax','PV','GP','$receiptdate','$sale_point_id','$trans_id','$master_id')";
					$this->db->query($queryd);
					$credit+=$vat_amount;
				 }
...
// new entries End 
		   ...
		   $sql_in="SELECT m.sino, d.itemid,d.quantity,i.itemname,d.amount,d.rate,d.inc_vat_amount,d.gstp FROM  tbl_goodsreceiving m,tbl_goodsreceiving_detail d ,tblmaterial_coding i where m.receiptnos=d.receipt_detail_id and
				...
				foreach($rw as $key=>$value) {
					...
					$nar1='Purchase Other #:'.$trans_id.',  '.$items_detail.'('.$remarks.')';
...
//}
	...
	public function manage_purchaseother($from,$to,$sale_point_id){
		...
		$this->db->where('tbl_goodsreceiving_detail.type','Other');
		$this->db->where('tbl_goodsreceiving_detail.category_id!=','1');
		$this->db->where('tbl_goodsreceiving.Purchase_type=','purchaseother');
	...
	public function edit_purchaseother($id){
	...
	public function update_purchase_other($data){
