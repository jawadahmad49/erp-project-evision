								...
								<div class="col-xs-12">
									...
									<div class="clearfix">
										...
										<div class="pull-right tableTools-container">
											<a href="<?php echo SURL ?>app/user/add_user">
									...
									<div>
										<table id="dynamic-table" class="table table-striped table-bordered table-hover">
											...
											foreach ($user_list as $user) {
												...
												$this->db->where_in('sale_point_id', $location_ids);
												$location_query = $this->db->get();
												$location_names = $location_query->result_array();

												$location_names_list = array_column($location_names, 'sp_name');
												$location_names_str = implode(', ', $location_names_list);
											?>

												<tr>
													<td>
														<?php echo $location_names_str; ?>
													</td>
													<td>
														<?php echo $user['admin_name']; ?>
													</td>
													<td>
														<?php echo $user['loginid']; ?>
													</td>
													<td class="hidden-480">
													...
													<td>
														<div class="hidden-sm hidden-xs action-buttons">
															<a class="green" href="<?php echo SURL . "app/user/edit/" . $user['id'] ?>">
																<i class="ace-icon fa fa-pencil bigger-130"></i>
															</a>
															<a id="bootbox-confirm" href="javascript:void(0)" class="red" onClick="confirmDelete('<?php echo SURL; ?>app/user/delete/<?php echo $user['id']; ?>');">
													...
													<?php if ($this->session->userdata('id') == 1) { ?>
														<td class="hidden-480">
															<button onclick="location.href = '<?php echo SURL . "app/user/role/" . $user['id'] ?>'" class="btn btn-xs btn-info">
																Access Role
															</button>
															<!-- <button onclick="location.href = '<?php echo SURL . "app/user/map_area/" . $user['id'] ?>'" class="btn btn-xs btn-info">
				Map Area
			</button> -->
														</td>
													<?php } ?>
													<?php if ($user['dashboard'] == "" || $user['dashboard'] == "Hide") { ?>
														<td><button name="show" class="btn btn-xs show btn-primary" value="<?php echo $user['id']; ?>" data-value="Show">Show</button></td>
													<?php } elseif ($user['dashboard'] == "Show") { ?>
														<td>
															<button name="hide" class="btn btn-xs hide_ btn-warning" value="<?php echo $user['id']; ?>" data-value="Hide">Hide</button>
														</td>
													<?php } ?>
												</tr>
...
<![endif]-->
	...
	<script type="text/javascript">
		$('.hide_').click(function() {
			...
			var request = $.ajax({
				url: "<?php echo SURL . "app/user/show_dashboard" ?>",
				type: "POST",
				data: {
					uid: id,
					status: st
			...
			request.done(function(msg) {
				...
				window.location.replace('<?php echo SURL . "app/User" ?>');
			});
