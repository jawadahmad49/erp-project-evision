...
<body class="no-skin">
									...
									<fieldset class="scheduler-border">
										...
										<div class="widget-main">
											...
											<div class="row">
												...
												<input type="hidden" name="total_orders" value="" />
												<input type="hidden" name="orders_delivered" value="" />
											</div>
										</div>
									</fieldset>
								</div>
	...
	<script type="text/javascript">
		...
		$(document).on('click', '.btnsubmit', function (e) {
			...
			if (status !== "Started") {
				if (!total_received || total_received === "0" || total_received === "0.00" || parseFloat(total_received) === 0) {
			...
			var total_orders = parseInt($("#total_orders").val(), 10) || 0;
			var orders_delivered = parseInt($("#orders_delivered").val(), 10) || 0;

			if (status !== "Started" && total_orders !== orders_delivered) {
				alert(`All orders are not delivered or dispatched! (Delivered: ${orders_delivered} / Total: ${total_orders})`);
				return false;
			}
		...
		function calculateTotalReceivable() {
			...
			const allInputs = $('input[id="grand_total"]');
			...
			$('#total_orders').val(totalInputs.toFixed(2));
			$('#orders_delivered').val(deliveredCount.toFixed(2));
		}
