                ...
                <table class="inventory">
                  ...
                  <tbody>
                    ...
                    foreach ($order_list_details as $value) {
                      ...
                      $is_new = ($value['type'] == 'New');

                      if ($value['type'] != 'Swap') {
                        $gst = round(($saleprice * $order_detail['gst']) / 100);
                        $ttl_gst += $value['quantity'] * $gst;
                        $base_amount = ($saleprice + $gst) * $value['quantity'];
                      }

                      $total_amount = $base_amount;

                      $repeat_times = $is_new ? 2 : 1;

                      for ($i = 0; $i < $repeat_times; $i++) {
                        if ($is_new && $i == 1) {
                          $total_amount = $base_amount + ($security_charges * $value['quantity']);
                          $ttl_security_charges += $security_charges * $value['quantity'];
                        } else {
                          $total_amount = $base_amount;
                        }
                        ?>
                        <tr id="row_<?php echo $value['id'] . ($i > 0 ? '_repeat' : '') ?>">
                          <td>
                            <?php
                            if ($value['type'] !== 'Swap' && $value['type'] !== 'New')
                              echo explode("-", $item_detail['itemname'])[0];
                            else if ($is_new && $i == 0 && $value['type'] == 'New')
                              echo explode("-", $item_detail['itemname'])[0];
                            else if ($is_new && $i == 1 && $value['type'] == 'New')
                              echo explode("-", $item_detail['itemname'])[0]. ' (Security Charges)';
                            ?>
                          </td>
                          ...
                          <td style="text-align-last: end;">
                            <?php if ($is_new && $i == 1)
                              echo number_format($security_charges); ?>
                          </td>
                          <td style="text-align-last: end;">
                            <?php
                            if ($value['type'] !== 'Swap' && $value['type'] !== 'New')
                          ...
                          <td style="text-align-last: end;">
                            ...
                            if ($value['type'] !== 'Swap' && $value['type'] !== 'New')
                              ...
                              echo number_format($security_charges * $value['quantity']);
                            ?>
                          </td>
                        </tr>
                        <?php
