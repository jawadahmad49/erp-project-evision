			...
			<div class="main-content-inner">
				<div class="breadcrumbs ace-save-state" id="breadcrumbs" style="font-weight: bold;">
					<ul class="breadcrumb">
						...
						<li class="active">Manage Dispatch Order <?php if ($arabic_check == 'Yes') { ?>(إدارة العنصر
							)<?php } ?></li>
					</ul><!-- /.breadcrumb -->
				...
				<div class="page-content">
					...
					<div class="row">
						<div class="col-xs-12">
							...
							<div class="row">
								<div class="col-xs-12">
									<h3 class="header smaller lighter blue">Manage Dispatch Order <?php if ($arabic_check == 'Yes') { ?>(إدارة العنصر
										)<?php } ?> </h3>
									...
									?>
									<form role="form" method="post" action="<?php echo SURL . "app/Today_Order_dispatch/filter" ?>" style="background-color:#fafafa;">
										...
										<div class="form-group">
											...
											<div class="col-sm-2">
												<div class="input-group">
													<input name="from" class="form-control date-picker" id="datepicker" type="text" data-date-format="yyyy-mm-dd" data-date-start-date="<?php echo $comp['financial_start_date']; ?>" data-date-end-date="<?php echo $comp['financial_end_date']; ?>" readonly required value="<?php echo date('Y-m-d'); ?>">
										...
										<div class="form-group">
											...
											<div class="col-sm-2">
												<div class="input-group">
													<input name="to" class="form-control date-picker" id="datepicker1" type="text" data-date-start-date="<?php echo $comp['financial_start_date']; ?>" data-date-end-date="<?php echo $comp['financial_end_date']; ?>" readonly data-date-format="yyyy-mm-dd" required value="<?php echo date('Y-m-d'); ?>">
													<span class="input-group-addon">
														<i class="fa fa-calendar bigger-110"></i>
													</span>
												</div>
									...
									<div class="clearfix" style="margin-top: 10px;">
										...
										<div class="pull-right tableTools-container">
											...
											<!-- <a href="<?php echo SURL ?>Order_dispatch/add_menu_item">
												<button autofocus class="btn btn-info">
										...
										<table id="dynamic-table" class="table table-striped table-bordered table-hover">
											...
											<tbody>
												...
												for ($i = 0; $i < count($item); $i++) {
													...
													$material = $this->db->query("select * from tblmaterial_coding where materialcode='$itemid'")->row_array();

													$issuenos = $item[$i]['issuenos'];
													$count++;
												...
												?>
													...
													<tr>
														...
														<td> <?php echo $item[$i]['id'] ?> </td>
														<td> <?php echo $item[$i]['order_date'] ?> </td>

														<td>
															<?php echo $material['itemname'] ?>
														</td>
														<td> <?php echo $item[$i]['unit'] ?> </td>
														<td> <?php echo $item[$i]['quantity'] ?> </td>
														<td> <?php echo $item[$i]['amount'] ?> </td>
														<td class="green">
															<strong> <?php echo $item[$i]['status'] ?></strong>
														</td>
														<td>
															<div class="hidden-sm hidden-xs action-buttons">

																<a class="btn btn-info btn-sm" class="" target="blank" title="Print Invoice" href="<?php echo SURL . "app/Today_Order_dispatch/detail_invoice/$issuenos" ?>">
																	Veiw Detail
																</a>
																<?php if ($item[$i]['issue_invoice'] == '' || is_null($item[$i]['issue_invoice'])) { ?>
																	<a target="blank" class="btn btn-info btn-sm" class="" title="Print Invoice" href="<?php echo SURL . "app/Order_dispatch/pdffile/$issuenos" ?>">
																		Generate Invioce
																	</a>
																<?php } ?>
															</div>
...
<![endif]-->
	...
	<script>
		function useractivity() {
			var text = 'Transfer Module';
			var action = 'Edit';
			// alert(res);
			$.ajax({
				url: "<?php echo SURL . "soceity/Transfer_module/user_activity_for_edit"; ?>",
				cache: false,
				type: "POST",
				data: {
					text: text,
		...
		function useractivity_search() {
			var text = 'Transfer Module';
			var action = 'Search in Transfer Module';
			// alert(res);
			$.ajax({
				url: "<?php echo SURL . "soceity/Transfer_module/user_activity_for_edit"; ?>",
				cache: false,
