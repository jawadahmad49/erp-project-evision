		...
		<script type="text/javascript">
			...
			$("#returngas").blur(function() {
				...
				if (total_return > amount_sum) {
					...
					$('#totalrecv').val(securityamt + net_amount.toFixed(2));
				}
			...
			function edit_row(no) {
				...
				//    $("#returns_text"+no).prop('readonly', true);
				// }else{
				// 	 $("#returns_text"+no).prop('readonly', false);
			...
			function add_row() {
				...
				var row = table.insertRow(table_len).outerHTML = "<tr id='row" + table_len + "'>" +
					...
					"<td id='item_row" + table_len + "' style='width:15% !important;'><select class='form-control disable' name='item[]' id='item_" + table_len + "' data-placeholder='Choose a Item...' required='required' tabindex='-1' disabled onchange='itemchange(this.value," + table_len + ")'><?php
																																																																										...
																																																																										?><option value='<?php echo $data['materialcode']; ?>'><?php
																																																																																								...
																																																																																								?></option><?php
																																																																																											...
																																																																																											?></select></td>" +
					"<td hidden id='catcode_row" + table_len + "'><input type='hidden' id='catcode_text" + table_len + "' readonly tabindex='-1' value='" + catcode + "'  name='catcode[]' class='catcode'></td>" +
					...
					"<td id='type_row" + table_len + "' style='width:10% !important;'><select disabled='disabled' class='form-control disable' tabindex='-1' name='type[]' id='type_" + table_len + "'><option value='sale'>Sale</option><option value='security'>Security</option><option value='refill'>Refill</option><option value='wo_sec'>WO SEC</option></select></td>" +
					...
					"<td id='item_return_row" + table_len + "' style='width:10% !important;'><select class='form-control disable' name='item_return[]' id='item_return_" + table_len + "' disabled tabindex='-1' data-placeholder='Choose Return...' required='required' ><?php
																																																																			foreach ($item_list as $key => $data) {
																																																																			?><option value='<?php echo $data['materialcode']; ?>'><?php
																																																																																	echo ucwords($data['itemname']);
																																																																																	?></option><?php
																																																																																				...
																																																																																				?></select></td>" +
					"<td id='returns_row" + table_len + "'><input type='text' id='returns_text" + table_len + "' readonly tabindex='-1' value='" + returns + "' name='returns[]'></td>" +
					...
					"<td  class='hidden-480'  id='stotal_row" + table_len + "'><input type='text' id='stotal_text" + table_len + "' readonly tabindex='-1' value='" + qty * security + "' class='" + security_sale_total + "' name='stotal[]'></td>" +
			...
			function totals_of_totals() {
				...
				$('#gas_amount_sum_total_text').text(total_sum_gas_amount.toFixed(2));
				...
				$('#total_sum_gst_amounttotal_text').text(total_sum_gst_amounttotal.toFixed(2));
				...
				$('.total_sum_input_sale').each(function(i, obj) {
					var input_value = parseInt($(this).val());
					//alert(input_value);
					total_sum_sale_amount = total_sum_sale_amount + input_value;

				});
				var total_sale_security = total_sum_sale_amount + total_sum_security_amount;
				$('#security_sum_total_text').text(total_sale_security.toFixed(0));

				$('#security_amt').val(total_sum_security_amount.toFixed(2));
				$('#sale_security_amt').val(total_sum_sale_amount.toFixed(2));


				var gas_amt_after = total_sum_gas_amount - total_sum_security_amount - total_sum_sale_amount;
				//alert(gas_amt_after);
				$('#total_gass_amount').val(gas_amt_after.toFixed(2));
				...
				$('#amount_sum_class_id').text(total_sum_amount.toFixed(0))
				$('.security_sum_input').each(function(i, obj) {
					var input_value = parseInt($(this).val());
					total_security = total_security + input_value;

				});
				$('#security_sum_text_id').val(total_security.toFixed(2))
				...
				// document.getElementById("gasamt").value=tgasamt;
				// document.getElementById("totalrecv").value=tsamt+tgasamt;
				document.getElementById("total_bill").value = tsamt + tgasamt.toFixed(2);
				document.getElementById("after_discount_amt").value = tgasamt - parseInt(document.getElementById("total_discount").value).toFixed(2);
				document.getElementById("net_receivable_gas").value = tgasamt - parseInt(document.getElementById("total_discount").value).toFixed(2);
				$("#tempty").show();
				...
				$('#inc_d_charges_amount').val(inc_d_charges_amount.toFixed(2));

				var total_discount = $('#total_discount').val();
				var inc_d_charges_amount = $('#inc_d_charges_amount').val();

				if (parseInt(total_discount) > parseInt(inc_d_charges_amount)) {
				...
				document.getElementById("net_receivable_gas").value = inc_d_charges_amount - total_discount;
				var after_discount_amt = $('#after_discount_amt').val();
				var security_amt = $('#security_amt').val();
				var total_bill = parseFloat(security_amt) + parseFloat(after_discount_amt);
				...
				var sale_security_amt = parseFloat($('#sale_security_amt').val());
				...
				$("#final_total_price").text(parseInt(fprice) + parseInt(price).toFixed(2));
				$("#security_total_price").text(parseInt(fsecurity) + parseInt(security).toFixed(2));
				$("#total_total_price").text(parseInt(ftotal) + parseInt(amounttotal).toFixed(2));
			...
			function cal_net_bill(total_discount) {
				...
				var discount_amount = inc_d_charges_amount - total_discount;
				$('#after_discount_amt').val(discount_amount.toFixed(2));
				$('#net_receivable_gas').val(discount_amount.toFixed(2));
				// document.getElementById("after_discount_amt").value=inc_d_charges_amount-total_discount;
				var return_rate = parseFloat($('#return_rate').val());
				var return_gas = parseFloat($('#return_gas').val());
			...
			function culculate_gst(gstp) {
				...
				var total_discount = $('#total_discount').val();
				var inc_d_charges_amount = document.getElementById("inc_d_charges_amount").value;
				if (inc_d_charges_amount == '') {
					inc_d_charges_amount = 0;
				}
		...
		<script type="text/javascript">
			...
			function CalAmountRecv() {
				...
				$('#totalrecv').val(totalrecv.toFixed(2));
			...
			function CalAmounts(no) {
				...
				document.getElementById("net_receivable_gas").value = tsamt + tgasamt - parseInt(document.getElementById("total_discount").value);
			...
			function checking() {
				...
				if (vendor == '0' || vendor == '' || vendor == '0.00') {
					...
					$("#qty").focus();
					return false;
				} else if (price == 0 || price == '' || price == '0.00' || isNaN(price)) {
					...
					$("#type").focus();
					return false;
				} else if (qty > 0 && type == 'sale' && (filledstock == '0' || filledstock == '')) {
					alert("Filled Stock is nill");
					return false;
				} else if (type == 'sale' && qty > filledstock) {
					alert("Please Enter Qty Less than Filled Stock");
					$("#qty").focus();
					$("#qty").val(0);
					return false;
				} else {
			...
			function savechecking(no) {
				...
				} else if (returns_text > 0 && type != 'refill') {
					alert("Please Select Type Refill!");
					$("#type_" + no).focus();
					return false;
				} else {
			...
			function get_cat_code() {
				...
				$.ajax({
					url: "<?php echo SURL . "SaleLPG/get_cat_code"; ?>",
					cache: false,
					type: "POST",
					data: {
						item: item
			...
			function change_pay_mode(pay_value) {
				...
				if (pay_value == 'Cash') {
					...
					$('.cheque_no').val('');
					$('#securityamt').val('0.00');
					$('#cylinder_sale_amt').val('0.00');
					$('#gasamt').val('0.00');
					$('#totalrecv').val('0.00');
