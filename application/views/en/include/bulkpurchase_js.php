...
function culculat_enter_amount(amount_value,bbbb) {
...
if(pay_mode=='cash'){
	...
	{
		alert('Amount should be not greater than Balance');
...
// var item_new_value=document.getElementById("item_"+no).value;
    ...
    unitcost.innerHTML="<input name='unitcost[]' type='text' id='unitcost_text"+no+"' value='"+unitcost_val+"'  onkeyup='CalAmounts("+no+")' pattern='^[0-9]+$' title='Only Numbers Allowed...' readonly required maxlength='5'>";
...
function delete_row(no)
{
	...
	if(table_len <= 3){
		//alert('<?php echo SURL."bulkpurchase/" ?>');
		window.location.replace('<?php echo SURL."bulkpurchase/" ?>');
	}
    ...
    if(table_len==0){
    	...
    	var request = $.ajax({
			url: "<?php echo SURL ?>/bulkpurchase/trans_delete",
			type: "POST",
...
//if(checkings!=2){
    ...
    var table=document.getElementById("data_table2");
    var table_len=(table.rows.length);
    var row = table.insertRow(table_len).outerHTML="<tr id='row"+table_len+"'><td style='display:none;' id='"+item+"' class='"+type+"'>"+emptystock+"</td><td id='item_row"+table_len+"' style='width:15% !important;'><select class='form-control disable' name='item[]' id='item_"+table_len+"' data-placeholder='Choose a Item...' required='required' disabled='disabled' onchange='itemchange(this.value,"+table_len+")'><?php 
					foreach ($item_list as $key => $data) {
						?><option value='<?php echo $data['tank_id']; ?>'><?php 
    							echo ucwords($data['tank_name']); 
    					?></option><?php 
    				} 
    				?></select></td><td id='qty_row"+table_len+"'><input type='text' id='qty_text"+table_len+"'  readonly='readonly' value='"+qty+"' name='qty[]'></td><td id='unitcost_row"+table_len+"'><input name='unitcost[]' type='text' id='unitcost_text"+table_len+"'  readonly='readonly' value='"+unitcost+"'></td><td   class='hidden-480'  id='gst_row"+table_len+"'><input name='gst[]' type='text' id='gst_text"+table_len+"' readonly='readonly' value='"+gst+"' ></td>       					<td  class='hidden-480' id='gstamount_row"+table_len+"'><input class='total_amount_class' name='gst_amount[]' type='text' id='gstamount_text"+table_len+"' readonly='readonly' value='"+gst_amount+"' ></td><td id='examount_row"+table_len+"'><input class='total_ex_amount_class' name='examount[]' type='text' id='examount_text"+table_len+"' readonly='readonly' value='"+examount+"' ></td><td  class='hidden-480' id='amount_row"+table_len+"'><input class='amount_class' name='amount[]' type='text' id='amount_text"+table_len+"' readonly='readonly' value='"+amount+"' ></td><td style='display: inline-flex; border: 0px;'><input type='button' id='edit_button"+table_len+"' value='Edit' class='btn btn-xs btn-success' onclick='edit_row("+table_len+")'><input type='button' id='save_button"+table_len+"' value='Save' class='btn btn-xs btn-warning' onclick='savechecking("+table_len+")' style='display:none'> <input type='button' value='Delete' class='btn btn-xs btn-danger' onclick='delete_row("+table_len+")'></td></tr>";
...
function change_pay_mode(pay_value) {
	...
	if(pay_value=='cash')
	{
		...
		var t_id=0;
		var request = $.ajax({
		  url: "<?php echo SURL ?>DirectGIRN/get_cash_in_hand_bal/"+t_id,
		  type: "POST",
		   data: {t_id:t_id},
		  dataType: "html"
		});
	...
	else
	{
		...
		//$('.enter_amount_class').val(0);
			...
			var dt=$('#id-date-picker-1').val();
				var ids=$('#id').val();
			var request = $.ajax({
				url: "<?php echo SURL ?>DirectGIRN/get_bank_bal/"+bankcode + '/'+dt+'/'+ ids,
			type: "POST",
...
function CalAmount()
	...
	document.getElementById('amount').value=Math.round(ucost*qty+gstp);
	document.getElementById('examount').value=Math.round(ucost*qty);
	document.getElementById('total_amount').value=Math.round(ucost*qty);
}
...
if (price11kg==0 ) {
	...
	alert('Please enter price 11kg');
	$("#price11kg").focus();
	return false;
}

if (isNaN(qty) && isNaN(ucost) || isNaN(gst) || isNaN(ereturn)) {
...
//if ((Type == 2 && PageCount == 0) || (Type == 2 && PageCount == '')) {
		...
		$("#ereturn").prop('required',true);
		//$("#addremove").prop('onclick',null);
		$("#addremove").prop('type','submit');

} else if (item=='' || price11kg=='' || qty=='' || ucost=='' || gst=='' || ereturn=='') {
		...
		$("#gst").prop('required',true);
		$("#ereturn").prop('required',true);
		 $("#price11kg").prop('required',true);
		//$("#addremove").prop('onclick',null);
		$("#addremove").prop('type','submit');
	
} else {

	 $("#price11kg").prop('readonly',true);
	 $("#price11kg").prop('required',true);
	 $("#item").prop("required",true);
		document.getElementById("addremove").removeAttribute("type");
		$("#addremove").prop('type','button');
...
if (isNaN(qty) || isNaN(ucost) || isNaN(gst) ) {
//if ((Type == 2 && PageCount == 0) || (Type == 2 && PageCount == '')) {
		//$("#item").prop('required',true);
		//$("#qty").prop('required',true);
		//$("#unitcost").prop('required',true);
...
function delete_record(deleteid,parentid,idd)
{
	...
	$('#discount').val('');
	$('#net_payable').val('');
	$('#enter_amount_cash').val('0');
	
	$('#cheque_no').val('');
	$('#bank_balance').val('');
	$('#id-date-picker-1').val('');
	$('#enter_amount_bank').val('0');

	$('#cash_balance').val('');

	var cash = $('#pay_mode').val('cash');

	change_pay_mode(cash);

	var request = $.ajax({
	  url: "<?php echo SURL ?>bulkpurchase/record_delete",
	  type: "POST",
...
$('#ereturn').blur(function() {
	...
	if(Number(ereturn) > Number(filledstock)){
		//$(".bootbox-confirm").show();
		//$(".modal-backdrop").show();
		document.getElementById("ereturn").value="";
