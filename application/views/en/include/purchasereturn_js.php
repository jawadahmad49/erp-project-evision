...
function delete_row(no)
{
    ...
    if(table_len==0){
    	...
    	window.location.replace('<?php echo SURL."Purchasereturn/" ?>');
    }
...
function checking(){
	...
	if (vendor=='') {
		...
		$("#qty").focus();
		return false;
	}else if(gasamt=="" || gasamt==0 || isNaN(gasamt)){
...
$('#data_table1 > #data_table2  > tr').each(function() {
	...
	var row_number= row_number_str.slice(-1);
		...
		var type_filled= $('#type_'+row_number).val();
	//alert(date);return false;
		...
		request.done(function(msg) {
			...
			else
			{
				...
				$("#stock_text"+row_number).val(empty_filled[1]);
			}
