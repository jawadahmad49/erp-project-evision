...
function culculat_gst(gstp) {
    ...
    var inc_vat_amount= parseFloat($('#inc_vat_amount').val());
    var total_bill=parseFloat(inc_vat_amount)-parseFloat(return_amount);
    $('#netpayable').val(total_bill.toFixed(2));	
...
function culculat_enter_amount(amount_value) {
	...
	if(amount_value>net_payable){
		...
		if(confirm('Cash Balance is less then payment amount, Are you sure to make this transaction !!')){
		}else{
			$('.enter_amount_class').val(0);
			return false;	
		}
	...
	if(amount_value>net_payable){
		...
		if(confirm('Balance is less then payment amount, Are you sure to make this transaction !!')){
		}else{
			$('.enter_amount_class').val(0);
			return false;	
		}	
...
function edit_row(no)
{
    ...
    var typess=document.getElementById("type_"+no).value;
...
// var item_new_value=document.getElementById("item_"+no).value;
    ...
    ereturn.innerHTML="<input maxlength='6' name='ereturn[]' type='text' id='ereturn_text"+no+"' value='"+ereturn_val+"'  onkeyup='' pattern='^[0-9]+$' title='Only Numbers Allowed...' required>";

    emptystock.innerHTML="<input type='text' id='emptystock_text"+no+"' value='"+totalstock+"'   disabled='disabled'>";

    gstamount.innerHTML="<input class='total_amount_class' name='gst_amount[]' type='text' id='gstamount_text"+no+"' value='"+gstamount_val+"'  onkeyup='CalAmounts("+no+")' disabled='disabled'>";
    amount.innerHTML="<input name='amount[]' class='amount_class' type='text' id='amount_text"+no+"' value='"+amount_val+"'  onkeyup='CalAmounts("+no+")' disabled='disabled'>";
    examount.innerHTML="<input class='total_ex_amount_class' name='examount[]' type='text' id='examount_text"+no+"' value='"+examount_val+"'  onkeyup='CalAmounts("+no+")' disabled='disabled'>";

  $('.table>tbody>tr>td>input').addClass('form-control');
  $('.table>tbody>tr>td>select').addClass('form-control');
  $("#qty_text"+no).focus();
     var empty_return = $("#empty_return").val();
      if(empty_return=='true'){
	   $("#ereturn_text"+no).prop('readonly', true);
	}else{
		 $("#ereturn_text"+no).prop('readonly', false);
...
function save_row(no)
{
	...
	if (isNaN(ereturn_val)) {
		alert("Please Enter Only Number!");
		$("#ereturn_text"+no).focus();
		return false;
	...
	////////////////////JUST FOR THIS TIME///////////////////////
	// if(Number(ereturn_val) > Number(emptystock_val)){
	// 	document.getElementById("ereturn_text"+no).value="";
	// 	document.getElementById("ereturn_text"+no).focus();
	// 	alert("Return Items Should not greater than Empty Stock?");
	// 	return false;
	// }
	////////////////////////////////////////////////////////////	
	...
	var typess=document.getElementById("type_"+no).value;
if(typess=='Filled'){
	var totalstock = parseInt(emptystock_val)-parseInt(ereturn_val);
}else{
	var totalstock = parseInt(qty_val)+parseInt(emptystock_val)-parseInt(ereturn_val);
}
...
$('#data_table1 > #data_table2  > tr').each(function() {
    ...
    document.getElementById("ereturn_row"+no).innerHTML="<input  type='text' id='ereturn_text"+no+"' name='ereturn[]' value='"+ereturn_val+"' readonly='readonly'>";

    document.getElementById("emptystock_row"+no).innerHTML="<input  type='text' id='emptystock_text"+no+"' name='emptystock[]' value='"+totalstock+"' readonly='readonly'>";

    document.getElementById("gstamount_row"+no).innerHTML="<input class='total_amount_class' type='text' id='gstamount_text"+no+"' name='gst_amount[]' value='"+gstamount_val+"' readonly='readonly'>";

    document.getElementById("examount_row"+no).innerHTML="<input class='total_ex_amount_class' type='text' id='examount_text"+no+"' name='examount[]' value='"+examount_val+"' readonly='readonly'>";

    document.getElementById("amount_row"+no).innerHTML="<input class='amount_class' type='text' id='amount_text"+no+"' name='amount[]' value='"+amount_val+"' readonly='readonly'>";
...
function delete_row(no)
{
    ...
    if(table_len==0){
    	...
    	window.location.replace('<?php echo SURL."DirectGIRN/" ?>');

    	var deleteid = $('#edit_id').val();
    	var parentid =  document.getElementById("id").value;
    	var request = $.ajax({
			url: "<?php echo SURL ?>/DirectGIRN/trans_delete",
			type: "POST",
...
function add_row()
{
	...
	var typess=document.getElementById("type").value;
...
if(typess=='Filled'){
	var emptystock = parseInt(emptystock)-parseInt(ereturn);
}else{
	var emptystock = parseInt(emptystock)+parseInt(qty)-parseInt(ereturn);
}
	var qtys11=0;
	//var checkings=1;
...
//if(checkings!=2){
    ...
    var table=document.getElementById("data_table2");
    var table_len=(table.rows.length);
    var row = table.insertRow(table_len).outerHTML="<tr id='row"+table_len+"'><td style='display:none;' id='"+item+"' class='"+type+"'>"+emptystock+"</td><td id='item_row"+table_len+"' style='width:15% !important;'><select class='form-control disable' name='item[]' id='item_"+table_len+"' data-placeholder='Choose a Item...' required='required' disabled='disabled' tabindex='-1' onchange='itemchange(this.value,"+table_len+")'><?php 
					foreach ($item_list as $key => $data) {
						?><option value='<?php echo $data['materialcode']; ?>'><?php 
    							echo ucwords($data['itemname']); 
    					?></option><?php 
    				} 
    				?></select></td><td   class='hidden-480'  style='width:9% !important;'><select readonly='readonly' class='form-control' name='type[]' tabindex='-1' id='type_"+table_len+"'><option value='Filled'>Filled</option></select></td><td id='qty_row"+table_len+"'><input type='text' id='qty_text"+table_len+"'  readonly='readonly' tabindex='-1' value='"+qty+"' name='qty[]'></td><td id='unitcost_row"+table_len+"'><input name='unitcost[]' type='text' id='unitcost_text"+table_len+"'  readonly='readonly' tabindex='-1' value='"+unitcost+"'></td><td  hidden class='hidden-480'  id='gst_row"+table_len+"'><input name='gst[]' type='hidden' id='gst_text"+table_len+"' readonly='readonly' tabindex='-1' value='"+gst+"' ></td><td    class='hidden-480'  id='ereturn_row"+table_len+"'><input name='ereturn[]' type='text' id='ereturn_text"+table_len+"' readonly='readonly' tabindex='-1' value='"+ereturn+"' ></td>	<td   class='hidden-480'  id='emptystock_row"+table_len+"'><input type='text' id='emptystock_text"+table_len+"' readonly  tabindex='-1' value='"+emptystock+"' name='emptystock[]'></td><td hidden class='hidden-480' id='gstamount_row"+table_len+"'><input class='total_amount_class' name='gst_amount[]' type='hidden' id='gstamount_text"+table_len+"' readonly='readonly' tabindex='-1' value='"+gst_amount+"' ></td><td hidden id='examount_row"+table_len+"'><input class='total_ex_amount_class' name='examount[]' type='hidden' id='examount_text"+table_len+"' readonly='readonly' tabindex='-1' value='"+examount+"' ></td><td  class='hidden-480' id='amount_row"+table_len+"'><input class='amount_class' name='amount[]' type='text' id='amount_text"+table_len+"' readonly='readonly' tabindex='-1' value='"+amount+"' ></td><td style='display: inline-flex; border: 0px;'><input type='button' id='edit_button"+table_len+"' value='Edit' class='btn btn-xs btn-success editbtn' onclick='edit_row("+table_len+")'><input type='button' id='save_button"+table_len+"' value='Save' class='btn btn-xs btn-warning' onclick='savechecking("+table_len+")' style='display:none'> <input type='button' value='Delete' class='btn btn-xs btn-danger deletebtn' onclick='delete_row("+table_len+")'></td></tr>";
...
function qty_change(no)
{
	
	  var qty=document.getElementById("qty_text"+no).value;
	   $("#ereturn_text"+no).val(qty);
	   var empty_return = $("#empty_return").val();
	   if(empty_return=='true'){
	   $("#ereturn_text"+no).prop('readonly', true);
	}
...
if (vendor==0 || isNaN(vendor) || vendor=="") { 
	document.getElementById('err_message2').innerHTML='<div class="alert alert-danger"><button type="button" class="close" data-dismiss="alert"><i class="ace-icon fa fa-times"></i></button><strong><i class="ace-icon fa fa-times"></i>Oh snap!</strong>- Please select vendor first!</div>';
	 
	$("#vendor_choosen").focus();
	return false;
}else if (item==0 || isNaN(item) || item=="") { 
	// if(kg_11_price==0 || isNaN(kg_11_price) || kg_11_price==""){
		// document.getElementById('err_message2').innerHTML='<div class="alert alert-danger"><button type="button" class="close" data-dismiss="alert"><i class="ace-icon fa fa-times"></i></button><strong><i class="ace-icon fa fa-times"></i>Oh snap!</strong>- Please enter 11.8 kg price.</div>';
		// $("#kg_11_price").focus();
		// return false;
	// }
	...
	$("#qty").focus();
	return false;
}else if (ucost==0 || isNaN(ucost) || ucost=="") {
	document.getElementById('err_message2').innerHTML='<div class="alert alert-danger"><button type="button" class="close" data-dismiss="alert"><i class="ace-icon fa fa-times"></i></button><strong><i class="ace-icon fa fa-times"></i>Oh snap!</strong>- Please enter Rate!</div>';
	...
	return false;
		...
		document.getElementById("addremove").removeAttribute("type");
		$("#addremove").prop('type','button');
		$("#kg_11_price").prop("readonly",true);
		add_row();		
	}
	
	
	
	// else if(item !="" && kg_11_price==0){
	// //alert(kg_11_price);
	// document.getElementById('err_message2').innerHTML='<div class="alert alert-danger"><button type="button" class="close" data-dismiss="alert"><i class="ace-icon fa fa-times"></i></button><strong><i class="ace-icon fa fa-times"></i>Oh snap!</strong>- Please enter 11.8 kg price.</div>';
		// $("#kg_11_price").focus();
		// $("#item").val("");
...
document.getElementById('err_message2').innerHTML='';
 if (qty==0) {
	...
	return false;
		...
		//document.getElementById("#save_button"+no).removeAttribute("type");
...
function setting_submit(event){
	...
	if(pay_mode=='cash')
	{
		var enter_amount_cash = parseFloat($("#enter_amount_cash").val());
		var cash_balance = parseFloat($("#cash_balance").val());

 
		if(enter_amount_cash > cash_balance){
			alert("Amount must be less than Cash balance!");
			$("#enter_amount_cash").val('');
			$("#enter_amount_cash").focus();
			return false;
		}
	...
	{
		var enter_amount_bank = parseFloat($("#enter_amount_bank").val());
		var bank_balance = parseFloat($("#bank_balance").val());

		if(enter_amount_bank > bank_balance){
			alert("Amount must be less than Bank balance!");
			$("#enter_amount_bank").val('');
			$("#enter_amount_bank").focus();
			return false;
		}
		...
		if(check_no<=0 || check_no=='') {
			...
			return false;
		}else if(bank_balance=="" || bank_balance==0){
			alert("Bank balance must be greater then zero!");
			$("#bank_name").focus();
			return false;
		}
...
{
	var request = $.ajax({
	  url: "<?php echo SURL ?>DirectGIRN/record_delete",
	  type: "POST",
...
//alert(qtys11);
	...
	//alert(date);return false;
		...
		request.done(function(msg) {
			...
			$("#unitcost").val(new_price);
			}else{
				$("#unitcost").val(empty_filled[2]);
			}
		});
		request.fail(function(jqXHR, textStatus) {
		  alert( "Request failed: " + textStatus );
...
$('#data_table1 > #data_table2  > tr').each(function() {
	...
	var row_number= row_number_str.slice(-1);
	...
	//alert(date);return false;
		...
		request.done(function(msg) {
			...
			$("#emptystock_text"+row_number).val(empty_filled[1]);
...
//alert(qtys11);
	...
	//alert(date);return false;
		...
		request.done(function(msg) {
			...
			if(type=='Filled'){
				var filledstocked = myObj.filled;
				$("#emptystock").val(myObj.filled);
			   //var qtys112 =parseInt(filledstocked)-parseInt(qtys11);
			   //$("#emptystock").val(qtys112);
			   
			}else if(type=='Empty'){
				var emptystocked = myObj.empty;
				$("#emptystock").val(myObj.empty);
			   //var qtys1122 =parseInt(emptystocked)-parseInt(qtys11);
			   //$("#emptystock").val(qtys1122);
...
$('#ereturn').blur(function() {
	var ereturn = $("#ereturn").val();
	var stock_check = $("#stock_check").val();
	var filledstock = $("#emptystock").val();
	if(Number(ereturn) > Number(filledstock)  && stock_check=='true'){
		//$(".bootbox-confirm").show();
		//$(".modal-backdrop").show();
		document.getElementById("ereturn").value="";
		document.getElementById("ereturn").focus();
		alert("Return Items Should not greater than Empty Stock?");
		return false;
	}	
});
...
function itemchange(itemid,no){
	...
	//alert(date);return false;
		...
		request.done(function(msg) {
		   ...
		   $("#emptystock_text"+no).val(empty_filled[1]);
