...
function delete_row(no)
{
    ...
    if(table_len==0){
    	...
    	window.location.replace('<?php echo SURL."Emptysale/" ?>');
    }
...
$('#data_table1 > #data_table2  > tr').each(function() {
	...
	//alert(date);return false;
		...
		var request = $.ajax({
		  ...
		  type: "POST",
		  data: {item_id:item_id,date:date,Damage:Damage,type:type},
		  dataType: "html"
		});
		request.done(function(msg) { alert(msg);
			...
			$("#filledstock_text"+row_number).val(empty_filled[1]);
...
function add_row()
{
	...
	"<td id='item_row"+table_len+"' style='width:15% !important;'><select class='form-control disable' name='item[]' id='item_"+table_len+"' data-placeholder='Choose a Item...' required='required' disabled onchange='itemchange(this.value,"+table_len+")'><?php 
					...
					"<td  class='hidden-480'  id='gst_row"+table_len+"'><input type='text' id='gst_text"+table_len+"' readonly  value='"+gst+"' name='gst[]'></td>"+
...
{
	var request = $.ajax({
	  url: "<?php echo SURL ?>/Emptysale/record_delete",
	  type: "POST",
...
$('#item').on('change', function() {
	...
	//alert(date);return false;
		...
		var request = $.ajax({
		  url: "<?php echo SURL ?>Common/stock",
		  type: "POST",
		  data: {item_id:item_id,date:date,condition:condition,type:type},
		  dataType: "html"
		});
		request.done(function(msg) { 
			...
			if(type == "Filled"){
				...
				var empty_filled=msg.split('_');
				$("#filledstock").val(empty_filled[1]);
			 }
