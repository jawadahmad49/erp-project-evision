...
$('#data_table1 > #data_table2  > tr').each(function() {
    ...
    //document.getElementById("item_row"+no).innerHTML="<input style='width: 250px' type='text' id='item_text"+no+"' name='item' value='"+item_val+"' readonly>";
...
function delete_row(no)
{
    ...
    if(table_len==0){
    	...
    	window.location.replace('<?php echo SURL."Purchase_return_empty/" ?>');
    	
    	var deleteid = $('#edit_id').val();
    	var parentid =  document.getElementById("id").value;
    	var request = $.ajax({
			url: "<?php echo SURL ?>/Purchase_return_empty/trans_delete",
			type: "POST",
...
//if(checkings!=2){
    ...
    var table=document.getElementById("data_table2");
    var table_len=(table.rows.length);
    var row = table.insertRow(table_len).outerHTML="<tr id='row"+table_len+"'><td style='display:none;' id='"+item+"' class='"+type+"'>"+emptystock+"</td><td id='item_row"+table_len+"' style='width:15% !important;'><select class='form-control disable' name='item[]' tabindex='-1' id='item_"+table_len+"' data-placeholder='Choose a Item...' required='required' readonly onchange='itemchange(this.value,"+table_len+")'><?php 
					foreach ($item_list as $key => $data) {
						?><option value='<?php echo $data['materialcode']; ?>'><?php 
    							echo ucwords($data['itemname']); 
    					?></option><?php 
    				}
    				?></select></td><td class='hidden-480'  style='width:9% !important;'><select readonly='readonly' class='form-control' tabindex='-1' name='type[]' id='type_"+table_len+"'><option value='Empty'>Empty</option></select></td><td id='qty_row"+table_len+"' style='width:13% !important;'><input type='text' id='qty_text"+table_len+"' readonly='readonly' tabindex='-1' value='"+qty+"' name='qty[]'></td><td id='unitcost_row"+table_len+"' style='width:13% !important;'><input name='unitcost[]' tabindex='-1' type='text' id='unitcost_text"+table_len+"'  readonly='readonly' value='"+unitcost+"'></td><td hidden class='hidden-480'  id='gst_row"+table_len+"'><input name='gst[]' tabindex='-1' type='hidden' id='gst_text"+table_len+"' readonly='readonly' value='"+gst+"' ></td><td hidden class='hidden-480'  id='gstamount_row"+table_len+"'><input name='gst_amount[]' tabindex='-1' type='hidden' id='gstamount_text"+table_len+"' readonly='readonly' value='"+gst_amount+"' ></td><td hidden id='examount_row"+table_len+"'><input name='examount[]' tabindex='-1'  type='hidden' id='examount_text"+table_len+"' readonly='readonly' value='"+examount+"' ></td><td class='hidden-480' id='amount_row"+table_len+"' style='width:13% !important;'><input class='amount_class' tabindex='-1' name='amount[]' type='text' id='amount_text"+table_len+"' readonly='readonly' value='"+amount+"' ></td><td style='display: inline-flex; border: 0px;'><input type='button' id='edit_button"+table_len+"' value='Edit' class='btn btn-xs btn-success editbtn ' onclick='edit_row("+table_len+")'><input type='button' id='save_button"+table_len+"' value='Save' class='btn btn-xs btn-warning' onclick='savechecking("+table_len+")' style='display:none'> <input type='button' value='Delete' class='btn btn-xs btn-danger deletebtn ' onclick='delete_row("+table_len+")'></td></tr>";
...
if (isNaN(qty) || isNaN(ucost) || isNaN(gst) ) {
		...
		alert("Quantity, Unit cost must not be empty.");
		return false;
		$("#save_button"+no).prop('type','submit');
...
{
	var request = $.ajax({
	  url: "<?php echo SURL ?>/Purchase_return_empty/record_delete",
	  type: "POST",
	  data: {deleteid:deleteid,parentid:parentid},//,parentid:parentid
	  dataType: "html"
	});
...
function itemchange(itemid,no){
	...
	//alert(date);return false;
		...
		request.done(function(msg) {
		   var objJSON = msg.split('_');
		   $("#emptystock_text"+no).val(objJSON[1]);
