		...
		<script type="text/javascript">
			function edit_row(no) {
				...
				var unit_price = document.getElementById("unit_price_row" + no);
				// alert(unit_price);
				var amount = document.getElementById("amount_row" + no);


				var qty_data = qty.innerHTML;
				var price_data = unit_price.innerHTML;
				// alert(no);
				var amount_data = amount.innerHTML;


				var qty_val = document.getElementById("qty_text" + no).value;
				var price_val = document.getElementById("unit_price_text" + no).value;
				var amount_val = document.getElementById("amount_text" + no).value;
				$('#row' + no + ' input[type=text]').remove();


				// Rewriting inputs after edit btn clicked
				qty.innerHTML = "<input type='text' id='qty_text" + no + "' name='qty[]' value='" + qty_val + "' pattern='^[0-9]+$' onkeyup='get_amount(" + no + ")' title='Only Numbers Allowed...' required maxlength='5'>";
				unit_price.innerHTML = "<input type='text' name = 'unit_price[]' id='unit_price_text" + no + "' value='" + price_val + "'  onkeyup='get_amount(" + no + ")'>";
				amount.innerHTML = "<input type='text' class='amount_total edit_row' id='amount_text" + no + "' name = 'amount[]' readonly value='" + amount_val + "' > ";
			...
			function get_amount(no) {
				alert('insode func get_amount');
				// var qty_value = parseInt($('#qty' + no).val()) || '0';
				// var unit_price_value = parseInt($('#unit_price' + no).val()) || '0';

				// var amount_text = qty_value * unit_price_value;
				// $('#amount_text' + no).val(amount_text);
			...
			function save_row(no) {
				...
				// Rewriting inputs after save btn clicked
				document.getElementById("qty_row" + no).innerHTML = "<input  type='text' class = 'Cal_amounts' id='qty_text" + no + "' name='qty[]' onkeyup='get_amount(" + no + ")' value='" + qty_val + "'  readonly>";
				document.getElementById("unit_price_row" + no).innerHTML = "<input type='text' class = ''Cal_amounts' id='unit_price_text" + no + "' name='unit_price[]' onkeyup='get_amount(" + no + ")' value='" + price_val + "'  readonly>";
				document.getElementById("amount_row" + no).innerHTML = "<input  type='text' class = ' saverow' id='amount_text" + no + "' name='amount[]' value='" + amount_val + "' readonly>";
			...
			function add_row() {
				...
				// Adding tbl row in the lower table after add row btn clicked
				row.outerHTML = "<tr id='row" + table_len + "'>\
                    <td style='display:none;' id='" + item + "' class='" + refillnew + "'></td>\
                    <td id='item_row" + table_len + "' style='width:62%'>\
                        <select class='form-control' disabled name='item[]' id='item_" + table_len + "' data-placeholder='Choose an Item...' required='required' readonly>\
                            <?php foreach ($item_list as $key => $data) { ?>\
                                <option value='<?php echo $data['materialcode']; ?>'><?php echo ucwords($data['itemname']); ?></option>\
                            <?php } ?>\
                        </select>\
                    </td>\
                    <td style='width:10%' id='qty_row" + table_len + "'>\
                        <input type='text'  id='qty_text" + table_len + "' readonly value='" + qty + "' name='qty[]'>\
                    </td>\
                    <td style='width:12%' id='unit_price_row" + table_len + "'>\
                        <input type='text' class='add_row'  id='unit_price_text" + table_len + "' readonly value='" + unit_price + "' name='unit_price[]'>\
                    </td>\
                    <td style='width:12%'id='amount_row" + table_len + "'>\
                        <input type='text' id='amount_text" + table_len + "' readonly value='" + amount + "' name='amount[]'>\
                    </td>\
                    <td style='width:12%'>\
                        <select readonly disabled class='form-control' name='refillnew[]' id='refillnew_" + table_len + "'>\
                            <option value='refill'>Refill</option>\
                            <option value='new'>New</option>\
                        </select>\
                    </td>\
                    <td style='display: inline-flex; border: 0px;width: 100%'>\
                        <input type='button' id='edit_button" + table_len + "' value='Edit' class='btn btn-xs btn-success' onclick='edit_row(" + table_len + ")'>\
                        <input type='button' id='save_button" + table_len + "' value='Save' class='btn btn-xs btn-warning' onclick='savechecking(" + table_len + ")' style='display:none'>\
                        <input type='button' value='Delete' class='btn btn-xs btn-danger' onclick='delete_row(" + table_len + ")'>\
                    </td>\
                </tr>";
		...
		<script>
			$(document).on('keyup', 'input[name="qty[]"]', function() {
				var qty = parseFloat($(this).closest('tr').find('input[name="qty[]"]').val()) || 0;
				var unit_price = parseFloat($(this).closest('tr').find('input[name="unit_price[]"]').val()) || 0;
