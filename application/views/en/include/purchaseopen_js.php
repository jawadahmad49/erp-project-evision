...
function delete_row(no)
{
    ...
    if(table_len==0){
    	...
    	window.location.replace('<?php echo SURL."Purchase_opening/" ?>');
    	
    	var deleteid = $('#edit_id').val();
    	var parentid =  document.getElementById("id").value;
    	var request = $.ajax({
			url: "<?php echo SURL ?>/Purchase_opening/trans_delete",
			type: "POST",
...
$('#data_table1 > #data_table2  > tr').each(function() {
	 	...
	 	if(item==itemids && type==types){
			...
			$('.table>tbody>tr>td>select').addClass('form-control');
	 		...
	 		if(type!=types){
	 		alert("Please Select Type You Choose Before!.");
			var itemvalue = $("#item_"+table_len+"").val(item);
			var typevalue = $("#type_"+table_len+"").val(type);
			document.getElementById("item").value="";
			document.getElementById("qty").value="0";
...
{
	var request = $.ajax({
	  url: "<?php echo SURL ?>/Purchase_opening/record_delete",
	  type: "POST",
	  data: {deleteid:deleteid,parentid:parentid},//,parentid:parentid
	  dataType: "html"
	});
