...
function edit_row(no)
{
    ...
    $("#category_id_"+no).attr("readonly",false);
...
//console.log();return false;
    ...
    //var security_val=document.getElementById("security_text"+no).value;
   	var stock_val=parseInt(document.getElementById("stock_text"+no).value)+parseInt(qty_val);
    var amounttotal_val=document.getElementById("amounttotal_text"+no).value;
    
    $('#row'+no+' input[type=text]').remove();

    //var stock_val=parseInt(stock_val)+parseInt(qty_val);
...
function delete_row(no)
{
    ...
    if(table_len==0){
    	...
    	window.location.replace('<?php echo SURL."purchasereturnother/" ?>');
    }
...
if(vendor==''){
	...
	alert("Please enter Unit price in numbers format...");
	$("#gasamt").val("");
	$("#gasamt").focus();
	return false;
} else {
...
{
	var request = $.ajax({
	  url: "<?php echo SURL ?>/purchasereturnother/record_delete",
	  type: "POST",
...
$('#data_table1 > #data_table2  > tr').each(function() {
	...
	//alert(date);return false;
		...
		request.done(function(msg) {
			...
			$("#stock_text"+row_number).val(empty_filled[0]);
...
$('#qty').blur(function() {
	...
	if(Number(qty) > Number(stock) && stock_check=='true'){
		//$(".bootbox-confirm").show();
		//$(".modal-backdrop").show();
		document.getElementById("qty").value="";
